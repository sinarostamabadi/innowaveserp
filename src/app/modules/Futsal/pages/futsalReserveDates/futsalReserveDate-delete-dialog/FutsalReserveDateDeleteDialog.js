...
export function FutsalReserveDateDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // FutsalReserveDates UI Context
  ...
  // FutsalReserveDates Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.futsalReserveDates.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteFutsalReserveDate = () => {
    // server request for deleting futsalReserveDate by id
    dispatch(actions.deleteFutsalReserveDate(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("FutsalReserveDate.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
      ...
      <Modal.Footer>
        <div>
          ...
          <button
            type="button"
            onClick={deleteFutsalReserveDate}
