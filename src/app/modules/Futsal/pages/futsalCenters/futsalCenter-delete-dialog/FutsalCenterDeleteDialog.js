...
export function FutsalCenterDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // FutsalCenters UI Context
  ...
  // FutsalCenters Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.futsalCenters.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteFutsalCenter = () => {
    // server request for deleting futsalCenter by id
    dispatch(actions.deleteFutsalCenter(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("FutsalCenter.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
