...
export const fetchFutsalDiscounts = (queryParams) => (dispatch) => {
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.futsalDiscountsFetched({
...
export const fetchFutsalDiscount = (id) => (dispatch) => {
  if (!id) {
    return dispatch(
      actions.futsalDiscountFetched({ futsalDiscountForEdit: undefined })
    );
  }
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.futsalDiscountFetched({ futsalDiscountForEdit: futsalDiscount })
      );
    })
    .catch((error) => {
...
export const createFutsalDiscount =
  (futsalDiscountForCreation) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
