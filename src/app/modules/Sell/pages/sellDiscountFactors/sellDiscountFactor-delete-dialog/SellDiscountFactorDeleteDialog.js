...
export function SellDiscountFactorDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // SellDiscountFactors UI Context
  ...
  // SellDiscountFactors Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.sellDiscountFactors.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteSellDiscountFactor = () => {
    // server request for deleting sellDiscountFactor by id
    dispatch(actions.deleteSellDiscountFactor(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("SellDiscountFactor.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
      ...
      <Modal.Footer>
        <div>
          ...
          <button
            type="button"
            onClick={deleteSellDiscountFactor}
