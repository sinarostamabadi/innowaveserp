...
export function SellDiscountDetailDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // SellDiscountDetails UI Context
  ...
  // SellDiscountDetails Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.sellDiscountDetails.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteSellDiscountDetail = () => {
    // server request for deleting sellDiscountDetail by id
    dispatch(actions.deleteSellDiscountDetail(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("SellDiscountDetail.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
      ...
      <Modal.Footer>
        <div>
          ...
          <button
            type="button"
            onClick={deleteSellDiscountDetail}
