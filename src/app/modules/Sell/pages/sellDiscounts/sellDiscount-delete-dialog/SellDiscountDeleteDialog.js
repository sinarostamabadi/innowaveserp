...
export function SellDiscountDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // SellDiscounts UI Context
  ...
  // SellDiscounts Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.sellDiscounts.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteSellDiscount = () => {
    // server request for deleting sellDiscount by id
    dispatch(actions.deleteSellDiscount(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("SellDiscount.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
