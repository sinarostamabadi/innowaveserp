...
export function SellPricingDetailDeleteDialog({ id, show, onHide }) {
  ...
  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.sellPricingDetails.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteSellPricingDetail = () => {
    // server request for deleting sellPricingDetail by id
    dispatch(actions.deleteSellPricingDetail(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("SellPricingDetail.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
