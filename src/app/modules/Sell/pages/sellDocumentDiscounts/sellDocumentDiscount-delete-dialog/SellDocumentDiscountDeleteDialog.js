...
export function SellDocumentDiscountDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // SellDocumentDiscounts UI Context
  ...
  // SellDocumentDiscounts Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.sellDocumentDiscounts.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteSellDocumentDiscount = () => {
    // server request for deleting sellDocumentDiscount by id
    dispatch(actions.deleteSellDocumentDiscount(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("SellDocumentDiscount.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
      ...
      <Modal.Footer>
        <div>
          ...
          <button
            type="button"
            onClick={deleteSellDocumentDiscount}
