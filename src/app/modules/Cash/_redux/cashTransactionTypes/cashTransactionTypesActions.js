...
import { cashTransactionTypesSlice, callTypes } from "./cashTransactionTypesSlice";
...
export const fetchCashTransactionTypes = (queryParams) => (dispatch) => {
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(
        actions.cashTransactionTypesFetched({ totalCount: TotalCount, entities: Items })  
      );
    })  
    .catch((error) => {
...
export const fetchCashTransactionType = (id) => (dispatch) => {
  if (!id) {
    return dispatch(actions.cashTransactionTypeFetched({ cashTransactionTypeForEdit: undefined }));
  }
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(actions.cashTransactionTypeFetched({ cashTransactionTypeForEdit: cashTransactionType }));
    })  
    .catch((error) => {
...
export const createCashTransactionType = (cashTransactionTypeForCreation) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
...
export const updateCashTransactionType = (cashTransactionType) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
