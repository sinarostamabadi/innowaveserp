...
export const fetchPromissoryNotes = (queryParams) => (dispatch) => {
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(
        actions.promissoryNotesFetched({ totalCount: TotalCount, entities: Items })  
      );
    })  
    .catch((error) => {
...
export const fetchPromissoryNote = (id) => (dispatch) => {
  if (!id) {
    return dispatch(actions.promissoryNoteFetched({ promissoryNoteForEdit: undefined }));
  }
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(actions.promissoryNoteFetched({ promissoryNoteForEdit: promissoryNote }));
    })  
    .catch((error) => {
...
export const createPromissoryNote = (promissoryNoteForCreation) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
