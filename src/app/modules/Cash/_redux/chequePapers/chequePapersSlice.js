
import { createSlice } from "@reduxjs/toolkit";
const initialChequePapersState = {
  listLoading: false,
  actionsLoading: false,
  totalCount: 0,
  entities: null,
  chequePaperForEdit: undefined,
  lastError: null,
  error: null,
};
export const callTypes = {
  list: "list",
  action: "action",
};
export const chequePapersSlice = createSlice({
  name: "chequePapers",
  initialState: initialChequePapersState,
  reducers: {
    catchError: (state, action) => {
      state.error = `${action.type}: ${action.payload.error}`;
      if (action.payload.callType === callTypes.list) {
        state.listLoading = false;
      } else {
        state.actionsLoading = false;
      }
    },
    startCall: (state, action) => {
      state.error = null;
      if (action.payload.callType === callTypes.list) {
        state.listLoading = true;
      } else {
        state.actionsLoading = true;
      }
    },
    // getChequePaperById  
    chequePaperFetched: (state, action) => {
      state.actionsLoading = false;
      state.chequePaperForEdit = action.payload.chequePaperForEdit;
      state.error = null;
    },
    // findChequePapers  
    chequePapersFetched: (state, action) => {
      const { entities, totalCount } = action.payload;
      state.listLoading = false;
      state.error = null;
      state.entities = entities;
      state.totalCount = totalCount;
    },
    // createChequePaper  
    chequePaperCreated: (state, action) => {
      state.actionsLoading = false;
      state.error = null;
      state.entities.push(action.payload);
    },
    // updateChequePaper  
    chequePaperUpdated: (state, action) => {
      state.error = null;
      state.actionsLoading = false;
      state.entities = state.entities.map((entity) => {
        if (entity.ChequePaperId === action.payload.chequePaper.ChequePaperId) {
          return action.payload.chequePaper;
        }
        return entity;
      });
    },
    // deleteChequePaper  
    chequePaperDeleted: (state, action) => {
      state.error = null;
      state.actionsLoading = false;
      state.entities = state.entities.filter(
        (el) => el.ChequePaperId !== action.payload.ChequePaperId  
      );
    },
    // deleteChequePapers  
    chequePapersDeleted: (state, action) => {
      state.error = null;
      state.actionsLoading = false;
      state.entities = state.entities.filter(
        (el) => !action.payload.ids.includes(el.ChequePaperId)  
      );
    },
    // chequePapersUpdateState  
    chequePapersStatusUpdated: (state, action) => {
      state.actionsLoading = false;
      state.error = null;
      const { ids, status } = action.payload;
      state.entities = state.entities.map((entity) => {
        if (ids.findIndex((id) => id === entity.ChequePaperId) > -1) {
          entity.status = status;
        }
        return entity;
      });
    },
  },
});
