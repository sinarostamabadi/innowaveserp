...
export const fetchPosTransactions = (queryParams) => (dispatch) => {
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(
        actions.posTransactionsFetched({ totalCount: TotalCount, entities: Items })  
      );
    })  
    .catch((error) => {
...
export const fetchPosTransaction = (id) => (dispatch) => {
  if (!id) {
    return dispatch(actions.posTransactionFetched({ posTransactionForEdit: undefined }));
  }
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(actions.posTransactionFetched({ posTransactionForEdit: posTransaction }));
    })  
    .catch((error) => {
...
export const createPosTransaction = (posTransactionForCreation) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
