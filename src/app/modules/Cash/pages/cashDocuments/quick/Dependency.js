...
const payment = {
  ...
  Clean: (dirty) => ({
    ...
    PaymentDate: dirty.PaymentDateObj
      ? FaObjToEnDateTime(dirty.PaymentDateObj)
      : dirty.PaymentDate,
    PaymentDateObj:
      !!dirty.PaymentDateObj == false && !!dirty.PaymentDate
        ? EnToFaObjDate(dirty.PaymentDate)
        : dirty.PaymentDateObj,
    Price: +dirty.Price,
...
const debt = {
  ...
  Clean: (dirty) => ({
    ...
    CashTransactionType: getTransactionType(dirty.TransactionTypeId),
    RequestDtlId: dirty.RequestDtlId,
    DebtDate: dirty.DebtDateObj
      ? FaObjToEnDateTime(dirty.DebtDateObj)
      : dirty.DebtDate,
    DebtDateObj:
      !!dirty.DebtDateObj == false && !!dirty.DebtDate
        ? EnToFaObjDate(dirty.DebtDate)
        : dirty.DebtDateObj,
    Price: +dirty.Price,
    Description: dirty.Description,
    IsDeleted: dirty.IsDeleted,
...
const pos = {
  ...
  CleanTran: (dirty, t) => ({
    ...
    TransactionTypeId: dirty.TransactionTypeId,
    Price: dirty.Price,
    Code: !!ToObject(dirty, "PosId", "Pos")
      ? ToObject(dirty, "PosId", "Pos").SerialNo
      : null,
  }),
};
...
const carToCard = {
  ...
  CleanTran: (dirty, t) => ({
    ...
    TransactionTypeId: dirty.TransactionTypeId,
    Price: dirty.Price,
    Code: !!ToObject(dirty, "BankCardId", "BankCard")
      ? ToObject(dirty, "BankCardId", "BankCard").CardNumber
      : null,
  }),
};
...
const coupon = {
  ...
  CleanTran: (dirty, t) => ({
    ...
    TransactionTypeId: dirty.TransactionTypeId,
    Price: dirty.Price,
    Code: !!ToObject(dirty, "CouponId", "Coupon")
      ? ToObject(dirty, "CouponId", "Coupon").CouponNumber
      : null,
  }),
};
...
const betweenBanks = {
  ...
  CleanTran: (dirty, t) => ({
    ...
    TransactionTypeId: dirty.TransactionTypeId,
    Price: dirty.Price,
    Code: !!ToObject(dirty, "BankId", "Bank")
      ? ToObject(dirty, "BankId", "Bank").TitleFa
      : null,
  }),
};
...
const cash = {
  ...
  Clean: (dirty, trans) => {
    const reciept =
      (!!dirty.Transactions.filter((x) => x.TransactionTypeId == 1).length
        ? dirty.Transactions.filter((x) => x.TransactionTypeId == 1)
            .map((x) => x.Price)
            .reduce((a, b) => a + b)
        : 0) + dirty.Added;
    const payment =
      (!!dirty.Transactions.filter((x) => x.TransactionTypeId == 2).length
        ? dirty.Transactions.filter((x) => x.TransactionTypeId == 2)
            .map((x) => x.Price)
            .reduce((a, b) => a + b)
        : 0) + dirty.Deficit;
    ...
    return {
      ...
      Payments: trans.Payments.map((x) =>
        PaymentTools.CleanApi({ ...x, CashDocumentId: dirty.CashDocumentId })
      ),
      Debts: trans.Debts.map((x) =>
        DebtTools.CleanApi({ ...x, CashDocumentId: dirty.CashDocumentId })
      ),
      BankTransfers: trans.BankTransfers.map((x) => ({
        ...x,
        BankTransferId: null,
      })),
      Cheques: trans.Cheques.map((x) => ({ ...x, ChequeId: null })),
      CouponTransactions: trans.CouponTransactions.map((x) => ({
        ...x,
        CouponTransactionId: null,
      })),
      PromissoryNotes: trans.PromissoryNotes.map((x) => ({
        ...x,
        PromissoryNoteId: null,
      })),
      Credits: trans.Credits.length
        ? trans.Credits.map((x) => {
            x.PersonId = ToValue(dirty, "PersonId");
            return x;
          })
        : [],
      Wallets: trans.Wallets.length
        ? trans.Wallets.map((x) => {
            x.PersonId = ToValue(dirty, "PersonId");
            return x;
          })
