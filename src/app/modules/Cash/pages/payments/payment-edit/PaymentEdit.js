...
export function PaymentEdit({
  ...
  const { actionsLoading, paymentForEdit, error } = useSelector(
    (state) => ({
  ...
  useEffect(() => {
    let _title = id ? "" : t("Common.Create") + " " + t("Payment.Entity");

    if (paymentForEdit && id) {
      _title = t("Common.Edit") + " " + paymentForEdit.TitleFa;
    }
  ...
  const savePayment = (values) => {
    if (!id) {
      dispatch(actions.createPayment(values))
        .then((arg) => {
          backToPaymentsList();
        })
        .catch((err) => {});
    } else {
      dispatch(actions.updatePayment(id, values))
        .then(() => backToPaymentsList())
        .catch((err) => {});
    }
  };
        ...
        <div className="mt-5">
          {tab === "basic" && (
            <PaymentEditForm
              ...
              payment={paymentForEdit || initModel}
              btnRef={btnRef}
