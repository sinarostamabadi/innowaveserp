...
export function BankTransferDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // BankTransfers UI Context
  ...
  // BankTransfers Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.bankTransfers.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteBankTransfer = () => {
    // server request for deleting bankTransfer by id
    dispatch(actions.deleteBankTransfer(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("BankTransfer.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
