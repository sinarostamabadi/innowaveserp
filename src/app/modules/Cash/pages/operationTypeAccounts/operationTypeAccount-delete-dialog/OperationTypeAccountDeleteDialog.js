...
export function OperationTypeAccountDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // OperationTypeAccounts UI Context
  ...
  // OperationTypeAccounts Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.operationTypeAccounts.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteOperationTypeAccount = () => {
    // server request for deleting operationTypeAccount by id
    dispatch(actions.deleteOperationTypeAccount(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("OperationTypeAccount.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
      ...
      <Modal.Footer>
        <div>
          ...
          <button
            type="button"
            onClick={deleteOperationTypeAccount}
