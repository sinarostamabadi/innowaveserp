...
import { masseurMassageTypesSlice, callTypes } from "./masseurMassageTypesSlice";
...
export const fetchMasseurMassageTypes = (queryParams) => (dispatch) => {
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(
        actions.masseurMassageTypesFetched({ totalCount: TotalCount, entities: Items })  
      );
    })  
    .catch((error) => {
...
export const fetchMasseurMassageType = (id) => (dispatch) => {
  if (!id) {
    return dispatch(actions.masseurMassageTypeFetched({ masseurMassageTypeForEdit: undefined }));
  }
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(actions.masseurMassageTypeFetched({ masseurMassageTypeForEdit: masseurMassageType }));
    })  
    .catch((error) => {
...
export const createMasseurMassageType = (masseurMassageTypeForCreation) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
