...
import {
  massageReservePricesSlice,
  callTypes,
} from "./massageReservePricesSlice";
...
export const fetchMassageReservePrices = (queryParams) => (dispatch) => {
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.massageReservePricesFetched({
...
export const fetchMassageReservePrice = (id) => (dispatch) => {
  if (!id) {
    return dispatch(
      actions.massageReservePriceFetched({
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.massageReservePriceFetched({
          massageReservePriceForEdit: massageReservePrice,
        })
      );
    })
    .catch((error) => {
...
export const createMassageReservePrice =
  (massageReservePriceForCreation) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
...
export const updateMassageReservePrice =
  (massageReservePrice) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
