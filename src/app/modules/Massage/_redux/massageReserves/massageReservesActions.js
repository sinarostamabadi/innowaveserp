...
export const fetchMassageReserves = (queryParams) => (dispatch) => {
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(
        actions.massageReservesFetched({ totalCount: TotalCount, entities: Items })  
      );
    })  
    .catch((error) => {
...
export const fetchMassageReserve = (id) => (dispatch) => {
  if (!id) {
    return dispatch(actions.massageReserveFetched({ massageReserveForEdit: undefined }));
  }
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(actions.massageReserveFetched({ massageReserveForEdit: massageReserve }));
    })  
    .catch((error) => {
...
export const createMassageReserve = (massageReserveForCreation) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
