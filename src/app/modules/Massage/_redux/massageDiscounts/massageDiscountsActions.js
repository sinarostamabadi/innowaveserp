...
export const fetchMassageDiscounts = (queryParams) => (dispatch) => {
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(
        actions.massageDiscountsFetched({ totalCount: TotalCount, entities: Items })  
      );
    })  
    .catch((error) => {
...
export const fetchMassageDiscount = (id) => (dispatch) => {
  if (!id) {
    return dispatch(actions.massageDiscountFetched({ massageDiscountForEdit: undefined }));
  }
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(actions.massageDiscountFetched({ massageDiscountForEdit: massageDiscount }));
    })  
    .catch((error) => {
...
export const createMassageDiscount = (massageDiscountForCreation) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
