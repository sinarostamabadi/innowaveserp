...
import { MasseurMassageTypeEditForm } from "./MasseurMassageTypeEditForm";
...
export function MasseurMassageTypeEdit({
  history,
  match: {
    params: { id },
  },
  ...
  const initModel = {
    MasseurMassageTypeId: undefined,
    TitleFa: "",
    TitleEn: "",
  };
  ...
  useEffect(() => {
    dispatch(actions.fetchMasseurMassageType(id));
  }, [id, dispatch]);
  ...
  }, [masseurMassageTypeForEdit, id]);

  const saveMasseurMassageType = (values) => {
    if (!id) {
      dispatch(actions.createMasseurMassageType(values))
        .then((arg) => {
  ...
  const btnRef = useRef();
  const saveMasseurMassageTypeClick = () => {
    if (btnRef && btnRef.current) {
      btnRef.current.click();
    }
  };

  const backToMasseurMassageTypesList = () => {
    ...
    <Card>
      ...
      <CardHeader title={title}>
        <CardHeaderToolbar>
          <button
            type="button"
            onClick={backToMasseurMassageTypesList}
          ...
          <button
            ...
            onClick={saveMasseurMassageTypeClick}
          >
      ...
      <CardBody>
        ...
        <div className="mt-5">
          {tab === "basic" && (
            <MasseurMassageTypeEditForm
              actionsLoading={actionsLoading}
              masseurMassageType={masseurMassageTypeForEdit || initModel}
              btnRef={btnRef}
              saveMasseurMassageType={saveMasseurMassageType}
            />
          )}
        </div>
      </CardBody>
