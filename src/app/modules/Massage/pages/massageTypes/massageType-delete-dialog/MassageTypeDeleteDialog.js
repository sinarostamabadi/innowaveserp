...
export function MassageTypeDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // MassageTypes UI Context
  ...
  // MassageTypes Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.massageTypes.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteMassageType = () => {
    // server request for deleting massageType by id
    dispatch(actions.deleteMassageType(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("MassageType.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
