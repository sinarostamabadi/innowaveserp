...
export function MasseurDeleteDialog({ id, show, onHide }) {
  ...
  // Masseurs Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.masseurs.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteMasseur = () => {
    // server request for deleting masseur by id
    dispatch(actions.deleteMasseur(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("Masseur.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
