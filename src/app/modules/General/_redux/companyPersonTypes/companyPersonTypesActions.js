...
export const fetchCompanyPersonTypes = (queryParams) => (dispatch) => {
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.companyPersonTypesFetched({
...
export const fetchCompanyPersonType = (id) => (dispatch) => {
  if (!id) {
    return dispatch(
      actions.companyPersonTypeFetched({ companyPersonTypeForEdit: undefined })
    );
  }
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.companyPersonTypeFetched({
          companyPersonTypeForEdit: companyPersonType,
        })
      );
    })
    .catch((error) => {
...
export const createCompanyPersonType =
  (companyPersonTypeForCreation) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
...
export const updateCompanyPersonType =
  (id, companyPersonType) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
