...
const initialCompanyPersonTypesState = {
  ...
  totalCount: 0,
  entities: null,
  companyPersonTypeForEdit: undefined,
  lastError: null,
  error: null,
};
...
export const companyPersonTypesSlice = createSlice({
  name: "companyPersonTypes",
  initialState: initialCompanyPersonTypesState,
  reducers: {
    ...
    companyPersonTypesStatusUpdated: (state, action) => {
      ...
      state.entities = state.entities.map((entity) => {
        if (ids.findIndex((id) => id === entity.CompanyPersonTypeId) > -1) {
