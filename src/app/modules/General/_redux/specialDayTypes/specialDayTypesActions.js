...
export const fetchSpecialDayTypes = (queryParams) => (dispatch) => {
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.specialDayTypesFetched({
...
export const fetchSpecialDayType = (id) => (dispatch) => {
  if (!id) {
    return dispatch(
      actions.specialDayTypeFetched({ specialDayTypeForEdit: undefined })
    );
  }
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.specialDayTypeFetched({ specialDayTypeForEdit: specialDayType })
      );
    })
    .catch((error) => {
...
export const createSpecialDayType =
  (specialDayTypeForCreation) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
