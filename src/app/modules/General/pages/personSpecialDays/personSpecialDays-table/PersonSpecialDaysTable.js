import React, { useEffect, useMemo } from "react";
import BootstrapTable from "react-bootstrap-table-next";
import paginationFactory, {
  PaginationProvider,
} from "react-bootstrap-table2-paginator";
import { shallowEqual, useDispatch, useSelector } from "react-redux";
import * as actions from "../../../_redux/personSpecialDays/personSpecialDaysActions";
import {
  getHandlerTableChange,
  NoRecordsFoundMessage,
  PleaseWaitMessage,
  sortCaret,
} from "../../../../../../core/_helpers";
import * as columnFormatters from "./column-formatters";
import { Pagination } from "../../../../../../core/_partials/controls";
import { usePersonSpecialDaysUIContext } from "../PersonSpecialDaysUIContext";
import { PersonSpecialDayModel } from "../../../../../../core/_models/General/PersonSpecialDayModel";
import {
  getConfig,
  getFields,
} from "../../../../../../core/_models/ModelDescriber";
import { useTranslation } from "react-i18next";

export function PersonSpecialDaysTable() {
  const { t } = useTranslation();

  const personSpecialDaysUIContext = usePersonSpecialDaysUIContext();

  const personSpecialDaysUIProps = useMemo(() => {
    return {
      ids: personSpecialDaysUIContext.ids,
      setIds: personSpecialDaysUIContext.setIds,
      queryParams: personSpecialDaysUIContext.queryParams,
      setQueryParams: personSpecialDaysUIContext.setQueryParams,
      openEditPersonSpecialDayPage:
        personSpecialDaysUIContext.openEditPersonSpecialDayPage,
      openDeletePersonSpecialDayDialog:
        personSpecialDaysUIContext.openDeletePersonSpecialDayDialog,
    };
  }, [personSpecialDaysUIContext]);

  const { currentState } = useSelector(
    (state) => ({ currentState: state.personSpecialDays }),
    shallowEqual
  );

  const { totalCount, entities, listLoading } = currentState;
  const configs = getConfig(PersonSpecialDayModel);
  const fieldKey = getFields(PersonSpecialDayModel);
  const fields = PersonSpecialDayModel;

  const dispatch = useDispatch();
  useEffect(() => {
    personSpecialDaysUIProps.setIds([]);
    dispatch(
      actions.fetchPersonSpecialDays(personSpecialDaysUIProps.queryParams)
    );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [personSpecialDaysUIProps.queryParams, dispatch]);

  const columns = [
    {
      dataField: fieldKey.TitleFa,
      text: t("PersonSpecialDay." + fields.TitleFa.display),
      sort: fields.TitleFa.sortable,
      sortCaret: sortCaret,
    },
    {
      dataField: fieldKey.TitleEn,
      text: t("PersonSpecialDay." + fields.TitleEn.display),
      sort: fields.TitleEn.sortable,
      sortCaret: sortCaret,
    },
    {
      dataField: "action",
      text: t("Common.Action"),
      formatter: columnFormatters.ActionsColumnFormatter,
      formatExtraData: {
        openEditPersonSpecialDayPage:
          personSpecialDaysUIProps.openEditPersonSpecialDayPage,
        openDeletePersonSpecialDayDialog:
          personSpecialDaysUIProps.openDeletePersonSpecialDayDialog,
        t: t,
      },
      classes: "text-right pr-0",
      headerClasses: "text-right pr-3",
      style: {
        minWidth: "100px",
      },
    },
  ];

  const paginationOptions = {
    custom: true,
    totalSize: totalCount,
    sizePerPageList: configs.sizePerPageList,
    sizePerPage: personSpecialDaysUIProps.queryParams.PageSize,
    page: personSpecialDaysUIProps.queryParams.PageNumber,
  };
  return (
    <>
      <PaginationProvider pagination={paginationFactory(paginationOptions)}>
        {({ paginationProps, paginationTableProps }) => {
          return (
            <Pagination
              isLoading={listLoading}
              paginationProps={paginationProps}
            >
              <BootstrapTable
                wrapperClasses="table-responsive"
                classes="table table-head-custom table-vertical-center"
                bootstrap4
                bordered={false}
                remote
                keyField={configs.id}
                data={entities === null ? [] : entities}
                columns={columns}
                defaultSorted={configs.defaultSorted}
                onTableChange={getHandlerTableChange(
                  personSpecialDaysUIProps.setQueryParams
                )}
                {...paginationTableProps}
              >
                <PleaseWaitMessage entities={entities} />
                <NoRecordsFoundMessage entities={entities} />
              </BootstrapTable>
            </Pagination>
          );
        }}
      </PaginationProvider>
    </>
  );
}
