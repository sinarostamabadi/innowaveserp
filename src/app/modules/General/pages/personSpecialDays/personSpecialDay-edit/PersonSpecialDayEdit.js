...
export function PersonSpecialDayEdit({
  history,
  match: {
    params: { id },
  },
  ...
  const { actionsLoading, personSpecialDayForEdit, error } = useSelector(
    (state) => ({
      ...
      error: state.personSpecialDays.error,
    }),
    shallowEqual
  );

  useEffect(() => {
    dispatch(actions.fetchPersonSpecialDay(id));
  }, [id, dispatch]);
  ...
  }, [personSpecialDayForEdit, id]);

  const savePersonSpecialDay = (values) => {
    if (!id) {
      dispatch(actions.createPersonSpecialDay(values))
        .then((arg) => {
          backToPersonSpecialDaysList();
        })
        .catch((err) => {});
    } else {
      dispatch(actions.updatePersonSpecialDay(id, values))
        .then(() => backToPersonSpecialDaysList())
        .catch((err) => {});
    }
  };

  const btnRef = useRef();
  const savePersonSpecialDayClick = () => {
    if (btnRef && btnRef.current) {
      btnRef.current.click();
    }
  };
  const backToPersonSpecialDaysList = () => {
      ...
      <CardHeader title={title}>
        <CardHeaderToolbar>
          ...
          <button
            ...
            onClick={savePersonSpecialDayClick}
          >
        ...
        <div className="mt-5">
          {tab === "basic" && (
            <PersonSpecialDayEditForm
              ...
              personSpecialDay={personSpecialDayForEdit || initModel}
              btnRef={btnRef}
