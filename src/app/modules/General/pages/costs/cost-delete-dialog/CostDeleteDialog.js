...
export function CostDeleteDialog({ id, show, onHide }) {
  ...
  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.costs.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteCost = () => {
    // server request for deleting cost by id
    dispatch(actions.deleteCost(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("Cost.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
