...
export function ProvinceDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // Provinces UI Context
  ...
  // Provinces Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.provinces.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteProvince = () => {
    // server request for deleting province by id
    dispatch(actions.deleteProvince(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("Province.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
