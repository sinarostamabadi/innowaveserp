...
export function CityEdit({
  ...
  const { actionsLoading, cityForEdit, error } = useSelector(
    (state) => ({
  ...
  useEffect(() => {
    let _title = id ? "" : t("Common.Create") + " " + t("City.Entity");

    if (cityForEdit && id) {
      _title = t("Common.Edit") + " " + cityForEdit.TitleFa;
    }
  ...
  const saveCity = (values) => {
    if (!id) {
      ...
      dispatch(actions.updateCity(id, values))
        .then(() => backToCitiesList())
        .catch((err) => {});
    }
  };
