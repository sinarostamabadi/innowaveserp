...
export function CountryEdit({
  ...
  const { actionsLoading, countryForEdit, error } = useSelector(
    (state) => ({
  ...
  useEffect(() => {
    let _title = id ? "" : t("Common.Create") + " " + t("Country.Entity");

    if (countryForEdit && id) {
      _title = t("Common.Edit") + " " + countryForEdit.TitleFa;
    }
        ...
        <div className="mt-5">
          {tab === "basic" && (
            <CountryEditForm
              ...
              country={countryForEdit || initModel}
              btnRef={btnRef}
