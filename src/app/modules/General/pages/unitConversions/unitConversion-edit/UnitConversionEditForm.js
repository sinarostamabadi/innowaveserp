...
export function UnitConversionEditForm({
  ...
  const UnitConversionEditSchema = Yup.object().shape({
    BaseUnitId: Yup.number().required(
      t("err.IsRequired", { 0: t("UnitConversion.BaseUnit") })
    ),
    ConvertedUnitId: Yup.number().required(
      t("err.IsRequired", { 0: t("UnitConversion.ConvertedUnit") })
    ),
    Amount: Yup.number().required(
      t("err.IsRequired", { 0: t("UnitConversion.Amount") })
    ),
  });
            ...
            <Form className="form form-label-right">
              <div className="form-group row">
                <div className="col-lg-6">
                  <Select
                    name="BaseUnitId"
                    label={t("UnitConversion.BaseUnitId")}
                  >
                ...
                <div className="col-lg-6">
                  <Select
                    name="ConvertedUnitId"
                    label={t("UnitConversion.ConvertedUnitId")}
                  >
                    {BaseUnit.map((child, index) => (
                      <option key={index} value={child.UnitId}>
                        {child.Name}
