...
export function SettingEdit({
  ...
  const { actionsLoading, settingForEdit, error } = useSelector(
    (state) => ({
  ...
  useEffect(() => {
    let _title = id ? "" : t("Common.Create") + " " + t("Setting.Entity");

    if (settingForEdit && id) {
      _title = t("Common.Edit") + " " + settingForEdit.TitleFa;
    }
  ...
  const saveSetting = (values) => {
    if (!id) {
      ...
      dispatch(actions.updateSetting(id, values))
        .then(() => backToSettingList())
        .catch((err) => {});
    }
  };
        ...
        <div className="mt-5">
          {tab === "basic" && (
            <SettingEditForm
              ...
              setting={settingForEdit || initModel}
              btnRef={btnRef}
