...
export function MajorDeleteDialog({ id, show, onHide }) {
  ...
  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.majors.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteMajor = () => {
    // server request for deleting major by id
    dispatch(actions.deleteMajor(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("Major.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
