...
export function DiscountTypeDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // DiscountTypes UI Context
  ...
  // DiscountTypes Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.discountTypes.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteDiscountType = () => {
    // server request for deleting discountType by id
    dispatch(actions.deleteDiscountType(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("DiscountType.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
