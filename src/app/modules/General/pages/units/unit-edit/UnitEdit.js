...
export function UnitEdit({
  ...
  const { actionsLoading, unitForEdit, error } = useSelector(
    (state) => ({
  ...
  useEffect(() => {
    let _title = id ? "" : t("Common.Create") + " " + t("Unit.Entity");

    if (unitForEdit && id) {
      _title = t("Common.Edit") + " " + unitForEdit.Name;
    }
  ...
  const saveUnit = (values) => {
    if (!id) {
      dispatch(actions.createUnit(values))
        .then((arg) => {
          backToUnitsList();
        })
        .catch((err) => { });
