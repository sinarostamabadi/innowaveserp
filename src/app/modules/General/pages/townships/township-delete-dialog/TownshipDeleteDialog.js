...
export function TownshipDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // Townships UI Context
  ...
  // Townships Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.townships.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteTownship = () => {
    // server request for deleting township by id
    dispatch(actions.deleteTownship(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("Township.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
