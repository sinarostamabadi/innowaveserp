...
export function EducationDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // Educations UI Context
  ...
  // Educations Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.educations.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteEducation = () => {
    // server request for deleting education by id
    dispatch(actions.deleteEducation(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("Education.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
