...
export function CoffeeShopDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // CoffeeShops UI Context
  ...
  // CoffeeShops Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.coffeeShops.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteCoffeeShop = () => {
    // server request for deleting coffeeShop by id
    dispatch(actions.deleteCoffeeShop(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("CoffeeShop.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
