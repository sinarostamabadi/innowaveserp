...
export function PersonGroupDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // PersonGroups UI Context
  ...
  // PersonGroups Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.personGroups.actionsLoading }),
    shallowEqual
  );
  ...
  const deletePersonGroup = () => {
    // server request for deleting personGroup by id
    dispatch(actions.deletePersonGroup(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("PersonGroup.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
