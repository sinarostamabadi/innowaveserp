...
export function CashDeleteDialog({ id, show, onHide }) {
  ...
  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.cashs.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteCash = () => {
    ...
    dispatch(actions.deleteCash(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("Cash.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
