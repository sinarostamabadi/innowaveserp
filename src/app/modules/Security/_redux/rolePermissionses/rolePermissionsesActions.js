...
export const fetchRolePermissionses = (queryParams) => (dispatch) => {
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(
        actions.rolePermissionsesFetched({ totalCount: TotalCount, entities: Items })  
      );
    })  
    .catch((error) => {
...
export const fetchRolePermissions = (id) => (dispatch) => {
  if (!id) {
    return dispatch(actions.rolePermissionsFetched({ rolePermissionsForEdit: undefined }));
  }
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(actions.rolePermissionsFetched({ rolePermissionsForEdit: rolePermissions }));
    })  
    .catch((error) => {
...
export const createRolePermissions = (rolePermissionsForCreation) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
