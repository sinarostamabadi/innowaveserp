...
export function ServicesDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // Serviceses UI Context
  ...
  // Serviceses Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.serviceses.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteServices = () => {
    // server request for deleting services by id
    dispatch(actions.deleteServices(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("Services.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
