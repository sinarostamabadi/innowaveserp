...
export function UserInRolesDeleteDialog({ id, show, onHide }) {
  ...
  // UserInRoleses Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.userInRoleses.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteUserInRoles = () => {
    // server request for deleting userInRoles by id
    dispatch(actions.deleteUserInRoles(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("UserInRoles.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
