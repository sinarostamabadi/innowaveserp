...
export function RolePermissionsDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // RolePermissionses UI Context
  ...
  // RolePermissionses Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.rolePermissionses.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteRolePermissions = () => {
    // server request for deleting rolePermissions by id
    dispatch(actions.deleteRolePermissions(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("RolePermissions.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
