...
import {
  bodyBuildingReservesSlice,
  callTypes,
} from "./bodyBuildingReservesSlice";
...
export const fetchBodyBuildingReserves = (queryParams) => (dispatch) => {
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.bodyBuildingReservesFetched({
...
export const fetchBodyBuildingReserve = (id) => (dispatch) => {
  if (!id) {
    return dispatch(
      actions.bodyBuildingReserveFetched({
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.bodyBuildingReserveFetched({
          bodyBuildingReserveForEdit: bodyBuildingReserve,
        })
      );
    })
    .catch((error) => {
...
export const createBodyBuildingReserve =
  (bodyBuildingReserveForCreation) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
...
export const updateBodyBuildingReserve =
  (bodyBuildingReserve) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
