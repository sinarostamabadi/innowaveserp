...
import {
  bodyBuildingDiscountsSlice,
  callTypes,
} from "./bodyBuildingDiscountsSlice";
...
export const fetchBodyBuildingDiscounts = (queryParams) => (dispatch) => {
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.bodyBuildingDiscountsFetched({
...
export const fetchBodyBuildingDiscount = (id) => (dispatch) => {
  if (!id) {
    return dispatch(
      actions.bodyBuildingDiscountFetched({
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.bodyBuildingDiscountFetched({
          bodyBuildingDiscountForEdit: bodyBuildingDiscount,
        })
      );
    })
    .catch((error) => {
...
export const createBodyBuildingDiscount =
  (bodyBuildingDiscountForCreation) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
...
export const updateBodyBuildingDiscount =
  (bodyBuildingDiscount) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
...
export const updateBodyBuildingDiscountsStatus =
  (ids, status) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
