...
export const PrintMenu = React.forwardRef(({ data }, ref) => {
  ...
  useEffect(() => {
    if(!!data){
      let sorted = _.orderBy(data, ["PlaceOfPreparation.Title", "RestaurantMenuGroup.Title", "BodyBuildingEmployee.NameFa"], ["asc", "asc", "asc"])
    ...
    return (menuItem.map((model) => (
      ...
      >
        ...
        <Barcode
          ...
          marginTop={2}
          displayValue={false}
          key={model.BodyBuildingEmployeeId}
          value={model.BodyBuildingEmployeeId.toString().padStart(6, "0")}
        />
      </div>
    )));
  }
