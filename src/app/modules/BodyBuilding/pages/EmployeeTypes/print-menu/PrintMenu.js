...
export const PrintMenu = React.forwardRef(({ data }, ref) => {
  ...
  useEffect(() => {
    if(!!data){
      let sorted = _.orderBy(data, ["PlaceOfPreparation.Title", "RestaurantMenuGroup.Title", "BodyBuildingEmployeeType.NameFa"], ["asc", "asc", "asc"])
    ...
    return (menuItem.map((model) => (
      ...
      >
        ...
        <Barcode
          ...
          marginTop={2}
          displayValue={false}
          key={model.BodyBuildingEmployeeTypeId}
          value={model.BodyBuildingEmployeeTypeId.toString().padStart(6, "0")}
        />
      </div>
    )));
  }
