...
import { BodyBuildingAccountTypeEditForm } from "./BodyBuildingAccountTypeEditForm";
...
export function BodyBuildingAccountTypeEdit({
  history,
  match: {
    params: { id },
  },
  ...
  const initModel = {
    BodyBuildingAccountTypeId: undefined,
    TitleFa: "",
    TitleEn: "",
  };
  ...
  useEffect(() => {
    dispatch(actions.fetchBodyBuildingAccountType(id));
  }, [id, dispatch]);
  ...
  }, [bodyBuildingAccountTypeForEdit, id]);

  const saveBodyBuildingAccountType = (values) => {
    if (!id) {
      dispatch(actions.createBodyBuildingAccountType(values))
        .then((arg) => {
          backToBodyBuildingAccountTypesList();
        })
        .catch((err) => {});
    } else {
      dispatch(actions.updateBodyBuildingAccountType(id, values))
        .then(() => backToBodyBuildingAccountTypesList())
        .catch((err) => {});
    }
  };

  const btnRef = useRef();
  const saveBodyBuildingAccountTypeClick = () => {
    if (btnRef && btnRef.current) {
      btnRef.current.click();
    }
  };

  const backToBodyBuildingAccountTypesList = () => {
    ...
    <Card>
      ...
      <CardHeader title={title}>
        <CardHeaderToolbar>
          <button
            type="button"
            onClick={backToBodyBuildingAccountTypesList}
