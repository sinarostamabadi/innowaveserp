...
export const PrintMenu = React.forwardRef(({ data }, ref) => {
  ...
  useEffect(() => {
    if(!!data){
      let sorted = _.orderBy(data, ["PlaceOfPreparation.Title", "RestaurantMenuGroup.Title", "BodyBuildingPack.NameFa"], ["asc", "asc", "asc"])
    ...
    return (menuItem.map((model) => (
      ...
      >
        ...
        <Barcode
          ...
          marginTop={2}
          displayValue={false}
          key={model.BodyBuildingPackId}
          value={model.BodyBuildingPackId.toString().padStart(6, "0")}
        />
      </div>
    )));
  }
