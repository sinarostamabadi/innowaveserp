...
export function ErrorHandlerDeleteDialog({ id, show, onHide }) {
  // ErrorHandlers UI Context  
  ...
  // ErrorHandlers Redux state  
  const dispatch = useDispatch();
  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.errorHandlers.actionsLoading }),
    shallowEqual  
  );
  ...
  const deleteErrorHandler = () => {
    // server request for deleting errorHandler by id  
    dispatch(actions.deleteErrorHandler(id)).then(() => {
