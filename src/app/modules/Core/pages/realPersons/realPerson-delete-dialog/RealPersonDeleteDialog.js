...
export function RealPersonDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();
  const [error, setError] = useState(null);

  // RealPersons UI Context
  ...
  const deleteRealPerson = () => {
    // server request for deleting realPerson by id
    dispatch(actions.deleteRealPerson(id))
      .then(() => {
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">
          {t("Common.Delete") + " " + t("RealPerson.Entity")}
        </Modal.Title>
      </Modal.Header>
      <Modal.Body>
