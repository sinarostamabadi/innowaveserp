...
import {
  coreTransactionTypesSlice,
  callTypes,
} from "./coreTransactionTypesSlice";
...
export const fetchCoreTransactionTypes = (queryParams) => (dispatch) => {
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.coreTransactionTypesFetched({
...
export const fetchCoreTransactionType = (id) => (dispatch) => {
  if (!id) {
    return dispatch(
      actions.coreTransactionTypeFetched({
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.coreTransactionTypeFetched({
          coreTransactionTypeForEdit: coreTransactionType,
        })
      );
    })
    .catch((error) => {
...
export const createCoreTransactionType =
  (coreTransactionTypeForCreation) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
...
export const updateCoreTransactionType =
  (coreTransactionType) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
