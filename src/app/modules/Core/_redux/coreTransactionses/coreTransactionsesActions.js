...
export const fetchCoreTransactionses = (queryParams) => (dispatch) => {
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(
        actions.coreTransactionsesFetched({ totalCount: totalCount, entities: items })  
      );
    })  
    .catch((error) => {
...
export const fetchCoreTransactions = (id) => (dispatch) => {
  if (!id) {
    return dispatch(actions.coreTransactionsFetched({ coreTransactionsForEdit: undefined }));
  }
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(actions.coreTransactionsFetched({ coreTransactionsForEdit: coreTransactions }));
    })  
    .catch((error) => {
...
export const createCoreTransactions = (coreTransactionsForCreation) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
