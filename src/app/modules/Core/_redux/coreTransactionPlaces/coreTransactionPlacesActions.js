...
import {
  coreTransactionPlacesSlice,
  callTypes,
} from "./coreTransactionPlacesSlice";
...
export const fetchCoreTransactionPlaces = (queryParams) => (dispatch) => {
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.coreTransactionPlacesFetched({
...
export const fetchCoreTransactionPlace = (id) => (dispatch) => {
  if (!id) {
    return dispatch(
      actions.coreTransactionPlaceFetched({
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.coreTransactionPlaceFetched({
          coreTransactionPlaceForEdit: coreTransactionPlace,
        })
      );
    })
    .catch((error) => {
...
export const createCoreTransactionPlace =
  (coreTransactionPlaceForCreation) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
...
export const updateCoreTransactionPlace =
  (coreTransactionPlace) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
...
export const updateCoreTransactionPlacesStatus =
  (ids, status) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
