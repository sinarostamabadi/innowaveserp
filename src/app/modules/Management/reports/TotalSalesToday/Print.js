  ...
  return (
    <>
      <div ref={ref} style={{ margin: "3rem" }}>
        ...
        >
          <caption>سرجمع فروش امروز : بستن صندوق</caption>
          <thead>
            <tr>
              <th className="border-dark" style={{ width: "5%" }}>
                کد
              </th>
              <th className="border-dark" style={{ width: "30%" }}>
                کالا
              </th>
              ...
              dataPrint.map((x, i) => (
                <tr key={x.ProductId}>
                  ...
                  <td className="border-dark ltr text-left">
                    {numberWithCommas(Math.ceil(x.Amount))}
                  </td>
                  <td className="border-dark ltr text-left">
                  ...
                  <td className="border-dark ltr text-left">
                    {numberWithCommas(Math.ceil(x.FinalPrice))}
                  </td>
                  <td className="border-dark ltr text-left">
                    {numberWithCommas(Math.ceil(x.Profit))}
                  </td>
                </tr>
              ))}
            <tr key="Total">
              <td className="border-dark" colSpan="5">
                جمع
              </td>
              <td className="border-dark ltr text-left">
                {numberWithCommas(
              ...
              <td className="border-dark ltr text-left">
                {numberWithCommas(
                  Math.ceil(
                    !!dataPrint &&
                      dataPrint.length &&
                      dataPrint.map((x) => x.FinalPrice).reduce((a, b) => a + b)
                  )
                )}
              </td>
              <td className="border-dark ltr text-left">
                {numberWithCommas(
                  Math.ceil(
                    !!dataPrint &&
                      dataPrint.length &&
                      dataPrint.map((x) => x.Profit).reduce((a, b) => a + b)
                  )
                )}
              </td>
            </tr>
