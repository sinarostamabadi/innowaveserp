              ...
              <th colSpan="8">
                <div className="flex">
                  <div className="flex-1">
                    {filterPrint &&
                      !!filterPrint.FromDate &&
                      "از تاریخ: " + EnToFaDateSlash(filterPrint.FromDate)}
                  </div>
                  <div className="flex-1">
                    {filterPrint &&
                      !!filterPrint.ToDate &&
                      "تا تاریخ : " + EnToFaDateSlash(filterPrint.ToDate)}
                  </div>
                </div>
              </th>
            </tr>
          ...
          <tbody>
            {!!dataPrint &&
              ...
              dataPrint.map((x, i) => (
                <>
                  ...
                  {filterPrint.HasDetail &&
                    !!x.SellDocumentDetails &&
                    x.SellDocumentDetails.length > 0 && (
                      <tr>
                        <td
                        ...
                        >
                          ...
                          >
                            ...
                            <tbody>
                              {!!x.SellDocumentDetails &&
                                x.SellDocumentDetails.length > 0 &&
                                x.SellDocumentDetails.map((detail, index) => (
                                  <tr key={detail.SellDocumentDetailId}>
                                    ...
                                    <td>
                                      {numberWithCommas(detail.PayablePrice)}
                                    </td>
                                    <td>
                                      {numberWithCommas(detail.FinalPrice)}
                                    </td>
            ...
            {!!dataPrint && dataPrint.length > 0 && (
              <tr style={{ backgroundColor: "#ccc" }}>
                <td colSpan="4" className="border-dark ltr text-right">
                  جمـــع کـــل:{" "}
                </td>
                <td className="border-dark ltr text-left">
                  {numberWithCommas(
                    Math.ceil(
                      dataPrint.map((y) => y.Price).reduce((a, b) => a + b)
                    )
                  )}
                </td>
                <td className="border-dark ltr text-left">
