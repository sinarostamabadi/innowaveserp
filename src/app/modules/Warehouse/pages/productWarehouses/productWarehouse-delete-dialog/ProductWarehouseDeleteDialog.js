...
export function ProductWarehouseDeleteDialog({ id, show, onHide }) {
  ...
  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.productWarehouses.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteProductWarehouse = () => {
    // server request for deleting productWarehouse by id
    dispatch(actions.deleteProductWarehouse(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("ProductWarehouse.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
