...
export function AssignmentDtlDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // AssignmentDtls UI Context
  ...
  // AssignmentDtls Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.assignmentDtls.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteAssignmentDtl = () => {
    // server request for deleting assignmentDtl by id
    dispatch(actions.deleteAssignmentDtl(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("AssignmentDtl.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
