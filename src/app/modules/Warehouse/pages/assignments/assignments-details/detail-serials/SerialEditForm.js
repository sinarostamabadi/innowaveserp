...
export function SerialEditForm({ detail, actionsLoading, onHide }) {
  ...
  const AssignmentSerialEditSchema = Yup.object().shape({
    SerialNo: Yup.string()
      .min(2, t("err.Min", { 0: 2 }))
      .max(500, t("err.Max", { 0: 100 }))
      .test(
        "checkSerial",
        t("err.DucplicateContent", { 0: t("AssignmentSerial.SerialNumber") }),
        function(value) {
          return !detailsUIProps.checkSerial(value);
        }
      )
      .required(t("err.IsRequired", { 0: t("AssignmentSerial.SerialNumber") })),
  });

  const columns = [
    {
      dataField: "SerialNumber",
      text: t("AssignmentSerial.SerialNumber"),
      sort: false,
    },
    {
      dataField: "action",
  ...
  function addSerial(values) {
    detailsUIProps.addSerial({
      AssignmentSerialId: "",
      AssignmentDtlId: detailsUIProps.selectedItem.AssignmentDtlId,
        ...
        <Row>
          <Col>
            <Formik
              ...
              validationSchema={AssignmentSerialEditSchema}
              onSubmit={(values) => {
                  ...
                  <Row>
                    <Col>
                      <Field
                        ...
                        component={Input}
                        customFeedbackLabel=""
                        label={t("AssignmentSerial.SerialNumber")}
                        setref={defaultInput}
                        onKeyPress={(e) => {
                          if (e.key === "Enter") {
                            e.stopPropagation();
        ...
        <Row>
          <Col>
            <BootstrapTable
              ...
              bootstrap4
              remote
              keyField="AssignmentSerialId"
              data={
                detailsUIProps.selectedItem.AssignmentSerials === null
                  ? []
                  : detailsUIProps.selectedItem.AssignmentSerials
              }
              columns={columns}
            >
              <PleaseWaitMessage
                entities={detailsUIProps.selectedItem.AssignmentSerials}
              />
              <NoRecordsFoundMessage
                entities={detailsUIProps.selectedItem.AssignmentSerials}
              />
