...
import { useInventoryOnTotalContext } from "./Context";
import {
  useReactToPrint,
  CloneObject,
  getStorage
...
export const Filter = forwardRef(({ reserve, btnRef, saveReserve }, ref) => {
  ...
  const context = useInventoryOnTotalContext();
  ...
  const InventoryOnTotalSchema = Yup.object().shape({
  ...
  const [InventoryOnTotalObj, setInventoryOnTotalObj] = useState(copyModel);
  ...
  return (
    <>
      <Formik
        ...
        initialValues={InventoryOnTotalObj}
        validationSchema={InventoryOnTotalSchema}
        onSubmit={(values) => {
          uiProps.setFilters({
            ...
            <Form className="form form-label-right">
              <div className="form-group row">
                <div className="col-lg-4">
                  <DefaultWarehouse
                    name="WarehouseId"
                    defaultWarehouse={InventoryOnTotalObj.Warehouse} />
                </div>
              </div>
              <button
                id="BtnSearchProduct"
