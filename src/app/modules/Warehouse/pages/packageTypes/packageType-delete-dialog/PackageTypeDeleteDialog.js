...
export function PackageTypeDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // PackageTypes UI Context
  ...
  // PackageTypes Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.packageTypes.actionsLoading }),
    shallowEqual
  );
  ...
  const deletePackageType = () => {
    // server request for deleting packageType by id
    dispatch(actions.deletePackageType(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("PackageType.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
