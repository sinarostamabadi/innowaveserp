...
export function AccountEdit({
  ...
  const { actionsLoading, accountForEdit, error } = useSelector(
    (state) => ({
  ...
  useEffect(() => {
    ...
    if (accountForEdit && id) {
      _title = t("Common.Edit") + " " + accountForEdit.TitleFa;
    }
  ...
  const saveAccount = (values) => {
    if (!id) {
      ...
      dispatch(actions.updateAccount(id, values))
        .then(() => backToAccountsList())
        .catch((err) => {});
    }
  };

  const btnRef = useRef();
  const saveAccountClick = () => {
        ...
        <div className="mt-5">
          {tab === "basic" && (
            <AccountEditForm
              ...
              account={accountForEdit || initModel}
              btnRef={btnRef}
