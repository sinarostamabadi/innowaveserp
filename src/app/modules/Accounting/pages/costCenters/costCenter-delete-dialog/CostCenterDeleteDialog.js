...
export function CostCenterDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // CostCenters UI Context
  ...
  // CostCenters Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.costCenters.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteCostCenter = () => {
    // server request for deleting costCenter by id
    dispatch(actions.deleteCostCenter(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("CostCenter.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
