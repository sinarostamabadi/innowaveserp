...
export function ImportAccountTempDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // ImportAccountTemps UI Context
  ...
  // ImportAccountTemps Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.importAccountTemps.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteImportAccountTemp = () => {
    // server request for deleting importAccountTemp by id
    dispatch(actions.deleteImportAccountTemp(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("ImportAccountTemp.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
      ...
      <Modal.Footer>
        <div>
          ...
          <button
            type="button"
            onClick={deleteImportAccountTemp}
