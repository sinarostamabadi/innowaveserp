...
export function ImportXMLSettingDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // ImportXMLSetting UI Context
  ...
  // ImportXMLSetting Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.importXMLSetting.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteImportXMLSetting = () => {
    // server request for deleting importXMLSetting by id
    dispatch(actions.deleteImportXMLSetting(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("ImportXMLSetting.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
      ...
      <Modal.Footer>
        <div>
          ...
          <button
            type="button"
            onClick={deleteImportXMLSetting}
