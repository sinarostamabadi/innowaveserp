...
import { AccountFloatingGroupTypeEditForm } from "./AccountFloatingGroupTypeEditForm";
...
export function AccountFloatingGroupTypeEdit({
  history,
  match: {
    params: { id },
  },
  ...
  }, [accountFloatingGroupTypeForEdit, id]);

  const saveAccountFloatingGroupType = (values) => {
    if (!id) {
      dispatch(actions.createAccountFloatingGroupType(values))
        .then((arg) => {
          backToAccountFloatingGroupTypesList();
        })
        .catch((err) => {});
    } else {
      dispatch(actions.updateAccountFloatingGroupType(id, values))
        .then(() => backToAccountFloatingGroupTypesList())
        .catch((err) => {});
    }
  };

  const btnRef = useRef();
  const saveAccountFloatingGroupTypeClick = () => {
    if (btnRef && btnRef.current) {
      btnRef.current.click();
    }
  };

  const backToAccountFloatingGroupTypesList = () => {
    ...
    <Card>
      ...
      <CardHeader title={title}>
        <CardHeaderToolbar>
          <button
            type="button"
            onClick={backToAccountFloatingGroupTypesList}
