...
export function AccountFloatingGroupTypeDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // AccountFloatingGroupTypes UI Context
  ...
  // AccountFloatingGroupTypes Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.accountFloatingGroupTypes.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteAccountFloatingGroupType = () => {
    // server request for deleting accountFloatingGroupType by id
    dispatch(actions.deleteAccountFloatingGroupType(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("AccountFloatingGroupType.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
      ...
      <Modal.Footer>
        <div>
          ...
          <button
            type="button"
            onClick={deleteAccountFloatingGroupType}
