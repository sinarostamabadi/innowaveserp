...
export function AccountTypeDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // AccountTypes UI Context
  ...
  // AccountTypes Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.accountTypes.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteAccountType = () => {
    // server request for deleting accountType by id
    dispatch(actions.deleteAccountType(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("AccountType.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
