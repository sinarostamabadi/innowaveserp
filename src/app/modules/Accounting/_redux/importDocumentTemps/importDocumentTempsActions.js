...
import {
  importDocumentTempsSlice,
  callTypes,
} from "./importDocumentTempsSlice";
...
export const fetchImportDocumentTemps = (queryParams) => (dispatch) => {
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.importDocumentTempsFetched({
...
export const fetchImportDocumentTemp = (id) => (dispatch) => {
  if (!id) {
    return dispatch(
      actions.importDocumentTempFetched({
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.importDocumentTempFetched({
          importDocumentTempForEdit: importDocumentTemp,
        })
      );
    })
    .catch((error) => {
...
export const createImportDocumentTemp =
  (importDocumentTempForCreation) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
