...
import { accountFloatingGroupTypesSlice, callTypes } from "./accountFloatingGroupTypesSlice";
...
export const fetchAccountFloatingGroupTypes = (queryParams) => (dispatch) => {
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(
        actions.accountFloatingGroupTypesFetched({ totalCount: TotalCount, entities: Items })  
      );
    })  
    .catch((error) => {
...
export const fetchAccountFloatingGroupType = (id) => (dispatch) => {
  if (!id) {
    return dispatch(actions.accountFloatingGroupTypeFetched({ accountFloatingGroupTypeForEdit: undefined }));
  }
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(actions.accountFloatingGroupTypeFetched({ accountFloatingGroupTypeForEdit: accountFloatingGroupType }));
    })  
    .catch((error) => {
...
export const createAccountFloatingGroupType = (accountFloatingGroupTypeForCreation) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
    .createAccountFloatingGroupType(accountFloatingGroupTypeForCreation)  
    .then((response) => {
      const accountFloatingGroupType = response.data;
      dispatch(actions.accountFloatingGroupTypeCreated(accountFloatingGroupType));
    })  
    .catch((error) => {
...
export const updateAccountFloatingGroupType = (id, accountFloatingGroupType) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
    .updateAccountFloatingGroupType(id, accountFloatingGroupType)  
    .then((response) => {
      dispatch(actions.accountFloatingGroupTypeUpdated({ accountFloatingGroupType }));
    })  
    .catch((error) => {
...
export const updateAccountFloatingGroupTypesStatus = (ids, status) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
    .updateStatusForAccountFloatingGroupTypes(ids, status)  
    .then(() => {
      dispatch(actions.accountFloatingGroupTypesStatusUpdated({ ids, status }));
    })  
    .catch((error) => {
