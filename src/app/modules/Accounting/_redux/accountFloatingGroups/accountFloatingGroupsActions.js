...
import {
  accountFloatingGroupsSlice,
  callTypes,
} from "./accountFloatingGroupsSlice";
...
export const fetchAccountFloatingGroups = (queryParams) => (dispatch) => {
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.accountFloatingGroupsFetched({
...
export const fetchAccountFloatingGroup = (id) => (dispatch) => {
  if (!id) {
    return dispatch(
      actions.accountFloatingGroupFetched({
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.accountFloatingGroupFetched({
          accountFloatingGroupForEdit: accountFloatingGroup,
        })
      );
    })
    .catch((error) => {
...
export const createAccountFloatingGroup =
  (accountFloatingGroupForCreation) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
...
export const updateAccountFloatingGroup =
  (id, accountFloatingGroup) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
