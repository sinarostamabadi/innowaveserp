...
export const fetchImportXMLSetting = (queryParams) => (dispatch) => {
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(
        actions.importXMLSettingFetched({ totalCount: TotalCount, entities: Items })  
      );
    })  
    .catch((error) => {
...
export const fetchImportXMLSetting = (id) => (dispatch) => {
  if (!id) {
    return dispatch(actions.importXMLSettingFetched({ importXMLSettingForEdit: undefined }));
  }
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(actions.importXMLSettingFetched({ importXMLSettingForEdit: importXMLSetting }));
    })  
    .catch((error) => {
      error.clientMessage = "Can't find importXMLSetting";
...
export const createImportXMLSetting = (importXMLSettingForCreation) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
