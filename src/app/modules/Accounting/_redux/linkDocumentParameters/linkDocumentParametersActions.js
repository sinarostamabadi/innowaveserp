...
import {
  linkDocumentParametersSlice,
  callTypes,
} from "./linkDocumentParametersSlice";
...
export const fetchLinkDocumentParameters = (queryParams) => (dispatch) => {
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.linkDocumentParametersFetched({
...
export const fetchLinkDocumentParameter = (id) => (dispatch) => {
  if (!id) {
    return dispatch(
      actions.linkDocumentParameterFetched({
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.linkDocumentParameterFetched({
          linkDocumentParameterForEdit: linkDocumentParameter,
        })
      );
    })
    .catch((error) => {
...
export const createLinkDocumentParameter =
  (linkDocumentParameterForCreation) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
...
export const updateLinkDocumentParameter =
  (linkDocumentParameter) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
      .updateLinkDocumentParameter(linkDocumentParameter)
      .then((response) => {
        dispatch(
          actions.linkDocumentParameterUpdated({ linkDocumentParameter })
        );
      })
      .catch((error) => {
...
export const updateLinkDocumentParametersStatus =
  (ids, status) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
