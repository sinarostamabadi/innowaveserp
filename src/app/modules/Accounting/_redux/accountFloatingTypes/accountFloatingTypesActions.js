...
import {
  accountFloatingTypesSlice,
  callTypes,
} from "./accountFloatingTypesSlice";
...
export const fetchAccountFloatingTypes = (queryParams) => (dispatch) => {
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.accountFloatingTypesFetched({
...
export const fetchAccountFloatingType = (id) => (dispatch) => {
  if (!id) {
    return dispatch(
      actions.accountFloatingTypeFetched({
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.accountFloatingTypeFetched({
          accountFloatingTypeForEdit: accountFloatingType,
        })
      );
    })
    .catch((error) => {
...
export const createAccountFloatingType =
  (accountFloatingTypeForCreation) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
...
export const updateAccountFloatingType =
  (id, accountFloatingType) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
