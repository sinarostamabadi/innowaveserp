...
export const fetchProfitLossItems = (queryParams) => (dispatch) => {
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(
        actions.profitLossItemsFetched({ totalCount: TotalCount, entities: Items })  
      );
    })  
    .catch((error) => {
...
export const fetchProfitLossItem = (id) => (dispatch) => {
  if (!id) {
    return dispatch(actions.profitLossItemFetched({ profitLossItemForEdit: undefined }));
  }
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(actions.profitLossItemFetched({ profitLossItemForEdit: profitLossItem }));
    })  
    .catch((error) => {
...
export const createProfitLossItem = (profitLossItemForCreation) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
