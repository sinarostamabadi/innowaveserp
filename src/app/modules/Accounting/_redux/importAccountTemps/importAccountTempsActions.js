...
export const fetchImportAccountTemps = (queryParams) => (dispatch) => {
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(
        actions.importAccountTempsFetched({ totalCount: TotalCount, entities: Items })  
      );
    })  
    .catch((error) => {
...
export const fetchImportAccountTemp = (id) => (dispatch) => {
  if (!id) {
    return dispatch(actions.importAccountTempFetched({ importAccountTempForEdit: undefined }));
  }
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(actions.importAccountTempFetched({ importAccountTempForEdit: importAccountTemp }));
    })  
    .catch((error) => {
...
export const createImportAccountTemp = (importAccountTempForCreation) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
