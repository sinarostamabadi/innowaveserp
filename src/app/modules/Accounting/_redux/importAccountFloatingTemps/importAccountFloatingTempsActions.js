...
import { importAccountFloatingTempsSlice, callTypes } from "./importAccountFloatingTempsSlice";
...
export const fetchImportAccountFloatingTemps = (queryParams) => (dispatch) => {
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(
        actions.importAccountFloatingTempsFetched({ totalCount: TotalCount, entities: Items })  
      );
    })  
    .catch((error) => {
...
export const fetchImportAccountFloatingTemp = (id) => (dispatch) => {
  if (!id) {
    return dispatch(actions.importAccountFloatingTempFetched({ importAccountFloatingTempForEdit: undefined }));
  }
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(actions.importAccountFloatingTempFetched({ importAccountFloatingTempForEdit: importAccountFloatingTemp }));
    })  
    .catch((error) => {
...
export const createImportAccountFloatingTemp = (importAccountFloatingTempForCreation) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
    .createImportAccountFloatingTemp(importAccountFloatingTempForCreation)  
    .then((response) => {
      const importAccountFloatingTemp = response.data;
      dispatch(actions.importAccountFloatingTempCreated(importAccountFloatingTemp));
    })  
    .catch((error) => {
...
export const updateImportAccountFloatingTemp = (importAccountFloatingTemp) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
    .updateImportAccountFloatingTemp(importAccountFloatingTemp)  
    .then((response) => {
      dispatch(actions.importAccountFloatingTempUpdated({ importAccountFloatingTemp }));
    })  
    .catch((error) => {
...
export const updateImportAccountFloatingTempsStatus = (ids, status) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
    .updateStatusForImportAccountFloatingTemps(ids, status)  
    .then(() => {
      dispatch(actions.importAccountFloatingTempsStatusUpdated({ ids, status }));
    })  
    .catch((error) => {
