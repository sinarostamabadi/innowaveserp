...
import { accountFloatingGroupRelationsSlice, callTypes } from "./accountFloatingGroupRelationsSlice";

const { actions } = accountFloatingGroupRelationsSlice;

export const fetchAccountFloatingGroupRelations = (queryParams) => (dispatch) => {
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.accountFloatingGroupRelationsFetched({ totalCount: TotalCount, entities: Items })
      );
    })
    .catch((error) => {
...
export const fetchAccountFloatingGroupRelation = (id) => (dispatch) => {
  if (!id) {
    return dispatch(actions.accountFloatingGroupRelationFetched({ accountFloatingGroupRelationForEdit: undefined }));
  }
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(actions.accountFloatingGroupRelationFetched({ accountFloatingGroupRelationForEdit: accountFloatingGroupRelation }));
    })
    .catch((error) => {
...
export const create = (accountFloatingGroupRelationForCreation) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer
    .create(accountFloatingGroupRelationForCreation)
    .then((response) => {
      const accountFloatingGroupRelation = response.data;
      dispatch(actions.accountFloatingGroupRelationCreated(accountFloatingGroupRelation));
