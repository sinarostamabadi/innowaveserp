...
import { CoffeeInvoiceCostEditForm } from "./CoffeeInvoiceCostEditForm";
...
export function CoffeeInvoiceCostEdit({
  history,
  match: {
    params: { id },
  },
  ...
  useEffect(() => {
    dispatch(actions.fetchCoffeeInvoiceCost(id));
  }, [id, dispatch]);
  ...
  }, [coffeeInvoiceCostForEdit, id]);

  const saveCoffeeInvoiceCost = (values) => {
    if (!id) {
      dispatch(actions.createCoffeeInvoiceCost(values))
        .then((arg) => {
  ...
  const btnRef = useRef();
  const saveCoffeeInvoiceCostClick = () => {
    if (btnRef && btnRef.current) {
      btnRef.current.click();
    }
  };

  const backToCoffeeInvoiceCostsList = () => {
    ...
    <Card>
      ...
      <CardHeader title={title}>
        <CardHeaderToolbar>
          <button
            type="button"
            onClick={backToCoffeeInvoiceCostsList}
          ...
          <button
            ...
            onClick={saveCoffeeInvoiceCostClick}
          >
      ...
      <CardBody>
        ...
        <div className="mt-5">
          {tab === "basic" && (
            <CoffeeInvoiceCostEditForm
              actionsLoading={actionsLoading}
              coffeeInvoiceCost={coffeeInvoiceCostForEdit || initModel}
              btnRef={btnRef}
              saveCoffeeInvoiceCost={saveCoffeeInvoiceCost}
            />
          )}
        </div>
      </CardBody>
