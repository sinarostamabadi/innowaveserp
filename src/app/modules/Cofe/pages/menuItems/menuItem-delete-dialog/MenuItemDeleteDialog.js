...
export function MenuItemDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // MenuItems UI Context
  ...
  // MenuItems Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.menuItems.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteMenuItem = () => {
    // server request for deleting menuItem by id
    dispatch(actions.deleteMenuItem(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("MenuItem.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
