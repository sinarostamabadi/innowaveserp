...
export function MenuGroupDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // MenuGroups UI Context
  ...
  // MenuGroups Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.menuGroups.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteMenuGroup = () => {
    // server request for deleting menuGroup by id
    dispatch(actions.deleteMenuGroup(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("MenuGroup.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
