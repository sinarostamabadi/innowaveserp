...
import { coffeeInvoiceDiscountsSlice, callTypes } from "./coffeeInvoiceDiscountsSlice";
...
export const fetchCoffeeInvoiceDiscounts = (queryParams) => (dispatch) => {
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(
        actions.coffeeInvoiceDiscountsFetched({ totalCount: TotalCount, entities: Items })  
      );
    })  
    .catch((error) => {
...
export const fetchCoffeeInvoiceDiscount = (id) => (dispatch) => {
  if (!id) {
    return dispatch(actions.coffeeInvoiceDiscountFetched({ coffeeInvoiceDiscountForEdit: undefined }));
  }
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(actions.coffeeInvoiceDiscountFetched({ coffeeInvoiceDiscountForEdit: coffeeInvoiceDiscount }));
    })  
    .catch((error) => {
...
export const createCoffeeInvoiceDiscount = (coffeeInvoiceDiscountForCreation) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
...
export const updateCoffeeInvoiceDiscount = (coffeeInvoiceDiscount) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
    .updateCoffeeInvoiceDiscount(coffeeInvoiceDiscount)  
    .then((response) => {
      dispatch(actions.coffeeInvoiceDiscountUpdated({ coffeeInvoiceDiscount }));
    })  
    .catch((error) => {
...
export const updateCoffeeInvoiceDiscountsStatus = (ids, status) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
