...
import { menuItemIngredientsesSlice, callTypes } from "./menuItemIngredientsesSlice";
...
export const fetchMenuItemIngredientses = (queryParams) => (dispatch) => {
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(
        actions.menuItemIngredientsesFetched({ totalCount: TotalCount, entities: Items })  
      );
    })  
    .catch((error) => {
...
export const fetchMenuItemIngredients = (id) => (dispatch) => {
  if (!id) {
    return dispatch(actions.menuItemIngredientsFetched({ menuItemIngredientsForEdit: undefined }));
  }
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(actions.menuItemIngredientsFetched({ menuItemIngredientsForEdit: menuItemIngredients }));
    })  
    .catch((error) => {
...
export const createMenuItemIngredients = (menuItemIngredientsForCreation) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
...
export const updateMenuItemIngredients = (menuItemIngredients) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
...
export const updateMenuItemIngredientsesStatus = (ids, status) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
