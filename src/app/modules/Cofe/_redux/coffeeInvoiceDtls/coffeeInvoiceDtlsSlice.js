...
const initialCoffeeInvoiceDtlsState = {
  ...
  totalCount: 0,
  entities: null,
  coffeeInvoiceDtlForEdit: undefined,
  lastError: null,
  error: null,
};
...
export const coffeeInvoiceDtlsSlice = createSlice({
  name: "coffeeInvoiceDtls",
  initialState: initialCoffeeInvoiceDtlsState,
  reducers: {
    ...
    coffeeInvoiceDtlsStatusUpdated: (state, action) => {
      ...
      state.entities = state.entities.map((entity) => {
        if (ids.findIndex((id) => id === entity.CoffeeInvoiceDtlId) > -1) {
