...
export const fetchCoffeeInvoiceDtls = (queryParams) => (dispatch) => {
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.coffeeInvoiceDtlsFetched({
...
export const fetchCoffeeInvoiceDtl = (id) => (dispatch) => {
  if (!id) {
    return dispatch(
      actions.coffeeInvoiceDtlFetched({ coffeeInvoiceDtlForEdit: undefined })
    );
  }
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.coffeeInvoiceDtlFetched({
          coffeeInvoiceDtlForEdit: coffeeInvoiceDtl,
        })
      );
    })
    .catch((error) => {
...
export const createCoffeeInvoiceDtl =
  (coffeeInvoiceDtlForCreation) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
