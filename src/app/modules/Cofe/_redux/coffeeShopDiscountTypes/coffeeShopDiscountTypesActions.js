...
import { coffeeShopDiscountTypesSlice, callTypes } from "./coffeeShopDiscountTypesSlice";
...
export const fetchCoffeeShopDiscountTypes = (queryParams) => (dispatch) => {
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(
        actions.coffeeShopDiscountTypesFetched({ totalCount: TotalCount, entities: Items })  
      );
    })  
    .catch((error) => {
...
export const fetchCoffeeShopDiscountType = (id) => (dispatch) => {
  if (!id) {
    return dispatch(actions.coffeeShopDiscountTypeFetched({ coffeeShopDiscountTypeForEdit: undefined }));
  }
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(actions.coffeeShopDiscountTypeFetched({ coffeeShopDiscountTypeForEdit: coffeeShopDiscountType }));
    })  
    .catch((error) => {
...
export const createCoffeeShopDiscountType = (coffeeShopDiscountTypeForCreation) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
...
export const updateCoffeeShopDiscountType = (coffeeShopDiscountType) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
    .updateCoffeeShopDiscountType(coffeeShopDiscountType)  
    .then((response) => {
      dispatch(actions.coffeeShopDiscountTypeUpdated({ coffeeShopDiscountType }));
    })  
    .catch((error) => {
...
export const updateCoffeeShopDiscountTypesStatus = (ids, status) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
