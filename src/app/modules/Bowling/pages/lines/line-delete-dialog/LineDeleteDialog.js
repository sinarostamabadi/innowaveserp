...
export function LineDeleteDialog({ id, show, onHide }) {
  ...
  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.lines.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteLine = () => {
    // server request for deleting line by id
    dispatch(actions.deleteLine(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("Line.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
