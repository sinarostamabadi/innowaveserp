...
export function ReserveDeleteDialog({ id, show, onHide }) {
  ...
  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.reserves.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteReserve = () => {
    ...
    dispatch(actions.deleteReserve(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("BowlingReserve.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
