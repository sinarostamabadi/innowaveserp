...
import {
  bowlingCompetitionsSlice,
  callTypes,
} from "./bowlingCompetitionsSlice";
...
export const fetchBowlingCompetitions = (queryParams) => (dispatch) => {
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.bowlingCompetitionsFetched({
...
export const fetchBowlingCompetition = (id) => (dispatch) => {
  if (!id) {
    return dispatch(
      actions.bowlingCompetitionFetched({
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.bowlingCompetitionFetched({
          bowlingCompetitionForEdit: bowlingCompetition,
        })
      );
    })
    .catch((error) => {
...
export const createBowlingCompetition =
  (bowlingCompetitionForCreation, fnCallBack) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
...
export const updateBowlingCompetition =
  (id, bowlingCompetition, fnCallBack) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
