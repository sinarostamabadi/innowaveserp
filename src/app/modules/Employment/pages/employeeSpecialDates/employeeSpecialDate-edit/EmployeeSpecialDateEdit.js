...
import { EmployeeSpecialDateEditForm } from "./EmployeeSpecialDateEditForm";
...
export function EmployeeSpecialDateEdit({
  history,
  match: {
    params: { id },
  },
  ...
  const initModel = {
    EmployeeSpecialDateId: undefined,
    TitleFa: "",
    TitleEn: "",
  };
  ...
  useEffect(() => {
    dispatch(actions.fetchEmployeeSpecialDate(id));
  }, [id, dispatch]);
  ...
  }, [employeeSpecialDateForEdit, id]);

  const saveEmployeeSpecialDate = (values) => {
    if (!id) {
      dispatch(actions.createEmployeeSpecialDate(values))
        .then((arg) => {
  ...
  const btnRef = useRef();
  const saveEmployeeSpecialDateClick = () => {
    if (btnRef && btnRef.current) {
      btnRef.current.click();
    }
  };

  const backToEmployeeSpecialDatesList = () => {
    ...
    <Card>
      ...
      <CardHeader title={title}>
        <CardHeaderToolbar>
          <button
            type="button"
            onClick={backToEmployeeSpecialDatesList}
          ...
          <button
            ...
            onClick={saveEmployeeSpecialDateClick}
          >
      ...
      <CardBody>
        ...
        <div className="mt-5">
          {tab === "basic" && (
            <EmployeeSpecialDateEditForm
              actionsLoading={actionsLoading}
              employeeSpecialDate={employeeSpecialDateForEdit || initModel}
              btnRef={btnRef}
              saveEmployeeSpecialDate={saveEmployeeSpecialDate}
            />
          )}
        </div>
      </CardBody>
