...
export function EmployeeEducarionDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // EmployeeEducarions UI Context
  ...
  // EmployeeEducarions Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.employeeEducarions.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteEmployeeEducarion = () => {
    // server request for deleting employeeEducarion by id
    dispatch(actions.deleteEmployeeEducarion(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("EmployeeEducarion.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
      ...
      <Modal.Footer>
        <div>
          ...
          <button
            type="button"
            onClick={deleteEmployeeEducarion}
