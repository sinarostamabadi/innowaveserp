...
import { EmployeeInIODeviceEditForm } from "./EmployeeInIODeviceEditForm";
...
export function EmployeeInIODeviceEdit({
  history,
  match: {
    params: { id },
  },
  ...
  const initModel = {
    EmployeeInIODeviceId: undefined,
    TitleFa: "",
    TitleEn: "",
  };
  ...
  useEffect(() => {
    dispatch(actions.fetchEmployeeInIODevice(id));
  }, [id, dispatch]);
  ...
  }, [employeeInIODeviceForEdit, id]);

  const saveEmployeeInIODevice = (values) => {
    if (!id) {
      dispatch(actions.createEmployeeInIODevice(values))
        .then((arg) => {
  ...
  const btnRef = useRef();
  const saveEmployeeInIODeviceClick = () => {
    if (btnRef && btnRef.current) {
      btnRef.current.click();
    }
  };

  const backToEmployeeInIODevicesList = () => {
    ...
    <Card>
      ...
      <CardHeader title={title}>
        <CardHeaderToolbar>
          <button
            type="button"
            onClick={backToEmployeeInIODevicesList}
          ...
          <button
            ...
            onClick={saveEmployeeInIODeviceClick}
          >
      ...
      <CardBody>
        ...
        <div className="mt-5">
          {tab === "basic" && (
            <EmployeeInIODeviceEditForm
              actionsLoading={actionsLoading}
              employeeInIODevice={employeeInIODeviceForEdit || initModel}
              btnRef={btnRef}
              saveEmployeeInIODevice={saveEmployeeInIODevice}
            />
          )}
        </div>
      </CardBody>
