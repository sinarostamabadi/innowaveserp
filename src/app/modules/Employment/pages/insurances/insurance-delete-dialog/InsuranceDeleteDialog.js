...
export function InsuranceDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // Insurances UI Context
  ...
  // Insurances Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.insurances.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteInsurance = () => {
    // server request for deleting insurance by id
    dispatch(actions.deleteInsurance(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("Insurance.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
