...
import { IODeviceTransactionTypeEditForm } from "./IODeviceTransactionTypeEditForm";
...
export function IODeviceTransactionTypeEdit({
  history,
  match: {
    params: { id },
  },
  ...
  const initModel = {
    IODeviceTransactionTypeId: undefined,
    TitleFa: "",
    TitleEn: "",
  };
  ...
  useEffect(() => {
    dispatch(actions.fetchIODeviceTransactionType(id));
  }, [id, dispatch]);
  ...
  }, [iODeviceTransactionTypeForEdit, id]);

  const saveIODeviceTransactionType = (values) => {
    if (!id) {
      dispatch(actions.createIODeviceTransactionType(values))
        .then((arg) => {
  ...
  const btnRef = useRef();
  const saveIODeviceTransactionTypeClick = () => {
    if (btnRef && btnRef.current) {
      btnRef.current.click();
    }
  };

  const backToIODeviceTransactionTypesList = () => {
    ...
    <Card>
      ...
      <CardHeader title={title}>
        <CardHeaderToolbar>
          <button
            type="button"
            onClick={backToIODeviceTransactionTypesList}
          ...
          <button
            ...
            onClick={saveIODeviceTransactionTypeClick}
          >
