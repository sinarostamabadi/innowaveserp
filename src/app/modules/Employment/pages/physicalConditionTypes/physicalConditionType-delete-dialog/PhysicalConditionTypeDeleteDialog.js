...
export function PhysicalConditionTypeDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // PhysicalConditionTypes UI Context
  ...
  // PhysicalConditionTypes Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.physicalConditionTypes.actionsLoading }),
    shallowEqual
  );
  ...
  const deletePhysicalConditionType = () => {
    // server request for deleting physicalConditionType by id
    dispatch(actions.deletePhysicalConditionType(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("PhysicalConditionType.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
      ...
      <Modal.Footer>
        <div>
          ...
          <button
            type="button"
            onClick={deletePhysicalConditionType}
