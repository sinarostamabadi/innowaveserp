...
import { SoldiershipExemptionEditForm } from "./SoldiershipExemptionEditForm";
...
export function SoldiershipExemptionEdit({
  history,
  match: {
    params: { id },
  },
  ...
  const initModel = {
    SoldiershipExemptionId: undefined,
    TitleFa: "",
    TitleEn: "",
  };
  ...
  useEffect(() => {
    dispatch(actions.fetchSoldiershipExemption(id));
  }, [id, dispatch]);
  ...
  }, [soldiershipExemptionForEdit, id]);

  const saveSoldiershipExemption = (values) => {
    if (!id) {
      dispatch(actions.createSoldiershipExemption(values))
        .then((arg) => {
  ...
  const btnRef = useRef();
  const saveSoldiershipExemptionClick = () => {
    if (btnRef && btnRef.current) {
      btnRef.current.click();
    }
  };

  const backToSoldiershipExemptionsList = () => {
    ...
    <Card>
      ...
      <CardHeader title={title}>
        <CardHeaderToolbar>
          <button
            type="button"
            onClick={backToSoldiershipExemptionsList}
          ...
          <button
            ...
            onClick={saveSoldiershipExemptionClick}
          >
      ...
      <CardBody>
        ...
        <div className="mt-5">
          {tab === "basic" && (
            <SoldiershipExemptionEditForm
              actionsLoading={actionsLoading}
              soldiershipExemption={soldiershipExemptionForEdit || initModel}
              btnRef={btnRef}
              saveSoldiershipExemption={saveSoldiershipExemption}
            />
          )}
        </div>
      </CardBody>
