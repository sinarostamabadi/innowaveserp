...
import { IOTransactionTypeEditForm } from "./IOTransactionTypeEditForm";
...
export function IOTransactionTypeEdit({
  history,
  match: {
    params: { id },
  },
  ...
  useEffect(() => {
    dispatch(actions.fetchIOTransactionType(id));
  }, [id, dispatch]);
  ...
  }, [iOTransactionTypeForEdit, id]);

  const saveIOTransactionType = (values) => {
    if (!id) {
      dispatch(actions.createIOTransactionType(values))
        .then((arg) => {
  ...
  const btnRef = useRef();
  const saveIOTransactionTypeClick = () => {
    if (btnRef && btnRef.current) {
      btnRef.current.click();
    }
  };

  const backToIOTransactionTypesList = () => {
    ...
    <Card>
      ...
      <CardHeader title={title}>
        <CardHeaderToolbar>
          <button
            type="button"
            onClick={backToIOTransactionTypesList}
          ...
          <button
            ...
            onClick={saveIOTransactionTypeClick}
          >
      ...
      <CardBody>
        ...
        <div className="mt-5">
          {tab === "basic" && (
            <IOTransactionTypeEditForm
              actionsLoading={actionsLoading}
              iOTransactionType={iOTransactionTypeForEdit || initModel}
              btnRef={btnRef}
              saveIOTransactionType={saveIOTransactionType}
            />
          )}
        </div>
      </CardBody>
