...
import { EmployeeInsuranceEditForm } from "./EmployeeInsuranceEditForm";
...
export function EmployeeInsuranceEdit({
  history,
  match: {
    params: { id },
  },
  ...
  useEffect(() => {
    dispatch(actions.fetchEmployeeInsurance(id));
  }, [id, dispatch]);
  ...
  }, [employeeInsuranceForEdit, id]);

  const saveEmployeeInsurance = (values) => {
    if (!id) {
      dispatch(actions.createEmployeeInsurance(values))
        .then((arg) => {
  ...
  const btnRef = useRef();
  const saveEmployeeInsuranceClick = () => {
    if (btnRef && btnRef.current) {
      btnRef.current.click();
    }
  };

  const backToEmployeeInsurancesList = () => {
    ...
    <Card>
      ...
      <CardHeader title={title}>
        <CardHeaderToolbar>
          <button
            type="button"
            onClick={backToEmployeeInsurancesList}
          ...
          <button
            ...
            onClick={saveEmployeeInsuranceClick}
          >
      ...
      <CardBody>
        ...
        <div className="mt-5">
          {tab === "basic" && (
            <EmployeeInsuranceEditForm
              actionsLoading={actionsLoading}
              employeeInsurance={employeeInsuranceForEdit || initModel}
              btnRef={btnRef}
              saveEmployeeInsurance={saveEmployeeInsurance}
            />
          )}
        </div>
      </CardBody>
