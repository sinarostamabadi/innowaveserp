...
import { MonthlyEmployeeIOEditForm } from "./MonthlyEmployeeIOEditForm";
...
export function MonthlyEmployeeIOEdit({
  history,
  match: {
    params: { id },
  },
  ...
  useEffect(() => {
    dispatch(actions.fetchMonthlyEmployeeIO(id));
  }, [id, dispatch]);
  ...
  }, [monthlyEmployeeIOForEdit, id]);

  const saveMonthlyEmployeeIO = (values) => {
    if (!id) {
      dispatch(actions.createMonthlyEmployeeIO(values))
        .then((arg) => {
  ...
  const btnRef = useRef();
  const saveMonthlyEmployeeIOClick = () => {
    if (btnRef && btnRef.current) {
      btnRef.current.click();
    }
  };

  const backToMonthlyEmployeeIOsList = () => {
    ...
    <Card>
      ...
      <CardHeader title={title}>
        <CardHeaderToolbar>
          <button
            type="button"
            onClick={backToMonthlyEmployeeIOsList}
          ...
          <button
            ...
            onClick={saveMonthlyEmployeeIOClick}
          >
      ...
      <CardBody>
        ...
        <div className="mt-5">
          {tab === "basic" && (
            <MonthlyEmployeeIOEditForm
              actionsLoading={actionsLoading}
              monthlyEmployeeIO={monthlyEmployeeIOForEdit || initModel}
              btnRef={btnRef}
              saveMonthlyEmployeeIO={saveMonthlyEmployeeIO}
            />
          )}
        </div>
      </CardBody>
