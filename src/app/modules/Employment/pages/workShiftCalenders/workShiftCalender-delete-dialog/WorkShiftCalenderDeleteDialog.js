...
export function WorkShiftCalenderDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // WorkShiftCalenders UI Context
  ...
  // WorkShiftCalenders Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.workShiftCalenders.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteWorkShiftCalender = () => {
    // server request for deleting workShiftCalender by id
    dispatch(actions.deleteWorkShiftCalender(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("WorkShiftCalender.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
      ...
      <Modal.Footer>
        <div>
          ...
          <button
            type="button"
            onClick={deleteWorkShiftCalender}
