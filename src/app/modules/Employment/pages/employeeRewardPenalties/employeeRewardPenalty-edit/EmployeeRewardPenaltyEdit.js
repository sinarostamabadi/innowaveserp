...
import { EmployeeRewardPenaltyEditForm } from "./EmployeeRewardPenaltyEditForm";
...
export function EmployeeRewardPenaltyEdit({
  history,
  match: {
    params: { id },
  },
  ...
  const initModel = {
    EmployeeRewardPenaltyId: undefined,
    TitleFa: "",
    TitleEn: "",
  };
  ...
  useEffect(() => {
    dispatch(actions.fetchEmployeeRewardPenalty(id));
  }, [id, dispatch]);
  ...
  }, [employeeRewardPenaltyForEdit, id]);

  const saveEmployeeRewardPenalty = (values) => {
    if (!id) {
      dispatch(actions.createEmployeeRewardPenalty(values))
        .then((arg) => {
  ...
  const btnRef = useRef();
  const saveEmployeeRewardPenaltyClick = () => {
    if (btnRef && btnRef.current) {
      btnRef.current.click();
    }
  };

  const backToEmployeeRewardPenaltiesList = () => {
    ...
    <Card>
      ...
      <CardHeader title={title}>
        <CardHeaderToolbar>
          <button
            type="button"
            onClick={backToEmployeeRewardPenaltiesList}
          ...
          <button
            ...
            onClick={saveEmployeeRewardPenaltyClick}
          >
      ...
      <CardBody>
        ...
        <div className="mt-5">
          {tab === "basic" && (
            <EmployeeRewardPenaltyEditForm
              actionsLoading={actionsLoading}
              employeeRewardPenalty={employeeRewardPenaltyForEdit || initModel}
              btnRef={btnRef}
              saveEmployeeRewardPenalty={saveEmployeeRewardPenalty}
            />
          )}
        </div>
      </CardBody>
