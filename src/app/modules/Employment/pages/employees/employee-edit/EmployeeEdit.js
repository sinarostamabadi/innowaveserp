...
export function EmployeeEdit({
  ...
  useEffect(() => {
    let _title = id ? "" : t("Common.Create") + " " + t("Employee.Entity");

    if (employeeForEdit && id) {
      _title = t("Common.Edit") + " " + employeeForEdit.TitleFa;
    }
  ...
  const saveEmployee = (values) => {
    if (!id) {
      ...
      dispatch(actions.updateEmployee(id, values))
        ...
        <div className="mt-5">
          {tab === "basic" && (
            <EmployeeEditForm
              ...
              employee={employeeForEdit || initModel}
              btnRef={btnRef}
