...
import { RewardOrPenaltyTypeEditForm } from "./RewardOrPenaltyTypeEditForm";
...
export function RewardOrPenaltyTypeEdit({
  history,
  match: {
    params: { id },
  },
  ...
  const initModel = {
    RewardOrPenaltyTypeId: undefined,
    TitleFa: "",
    TitleEn: "",
  };
  ...
  useEffect(() => {
    dispatch(actions.fetchRewardOrPenaltyType(id));
  }, [id, dispatch]);
  ...
  }, [rewardOrPenaltyTypeForEdit, id]);

  const saveRewardOrPenaltyType = (values) => {
    if (!id) {
      dispatch(actions.createRewardOrPenaltyType(values))
        .then((arg) => {
  ...
  const btnRef = useRef();
  const saveRewardOrPenaltyTypeClick = () => {
    if (btnRef && btnRef.current) {
      btnRef.current.click();
    }
  };

  const backToRewardOrPenaltyTypesList = () => {
    ...
    <Card>
      ...
      <CardHeader title={title}>
        <CardHeaderToolbar>
          <button
            type="button"
            onClick={backToRewardOrPenaltyTypesList}
          ...
          <button
            ...
            onClick={saveRewardOrPenaltyTypeClick}
          >
      ...
      <CardBody>
        ...
        <div className="mt-5">
          {tab === "basic" && (
            <RewardOrPenaltyTypeEditForm
              actionsLoading={actionsLoading}
              rewardOrPenaltyType={rewardOrPenaltyTypeForEdit || initModel}
              btnRef={btnRef}
              saveRewardOrPenaltyType={saveRewardOrPenaltyType}
            />
          )}
        </div>
      </CardBody>
