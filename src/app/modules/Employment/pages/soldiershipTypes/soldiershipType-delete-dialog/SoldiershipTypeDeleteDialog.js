...
export function SoldiershipTypeDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // SoldiershipTypes UI Context
  ...
  // SoldiershipTypes Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.soldiershipTypes.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteSoldiershipType = () => {
    // server request for deleting soldiershipType by id
    dispatch(actions.deleteSoldiershipType(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("SoldiershipType.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
