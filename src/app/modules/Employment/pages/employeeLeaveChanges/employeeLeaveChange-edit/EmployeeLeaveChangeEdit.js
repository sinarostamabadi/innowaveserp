...
import { EmployeeLeaveChangeEditForm } from "./EmployeeLeaveChangeEditForm";
...
export function EmployeeLeaveChangeEdit({
  history,
  match: {
    params: { id },
  },
  ...
  const initModel = {
    EmployeeLeaveChangeId: undefined,
    TitleFa: "",
    TitleEn: "",
  };
  ...
  useEffect(() => {
    dispatch(actions.fetchEmployeeLeaveChange(id));
  }, [id, dispatch]);
  ...
  }, [employeeLeaveChangeForEdit, id]);

  const saveEmployeeLeaveChange = (values) => {
    if (!id) {
      dispatch(actions.createEmployeeLeaveChange(values))
        .then((arg) => {
  ...
  const btnRef = useRef();
  const saveEmployeeLeaveChangeClick = () => {
    if (btnRef && btnRef.current) {
      btnRef.current.click();
    }
  };

  const backToEmployeeLeaveChangesList = () => {
    ...
    <Card>
      ...
      <CardHeader title={title}>
        <CardHeaderToolbar>
          <button
            type="button"
            onClick={backToEmployeeLeaveChangesList}
          ...
          <button
            ...
            onClick={saveEmployeeLeaveChangeClick}
          >
      ...
      <CardBody>
        ...
        <div className="mt-5">
          {tab === "basic" && (
            <EmployeeLeaveChangeEditForm
              actionsLoading={actionsLoading}
              employeeLeaveChange={employeeLeaveChangeForEdit || initModel}
              btnRef={btnRef}
              saveEmployeeLeaveChange={saveEmployeeLeaveChange}
            />
          )}
        </div>
      </CardBody>
