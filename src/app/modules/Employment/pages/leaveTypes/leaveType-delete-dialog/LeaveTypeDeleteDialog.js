...
export function LeaveTypeDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // LeaveTypes UI Context
  ...
  // LeaveTypes Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.leaveTypes.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteLeaveType = () => {
    // server request for deleting leaveType by id
    dispatch(actions.deleteLeaveType(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("LeaveType.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
