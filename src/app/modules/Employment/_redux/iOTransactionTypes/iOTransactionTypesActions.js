...
export const fetchIOTransactionTypes = (queryParams) => (dispatch) => {
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(
        actions.iOTransactionTypesFetched({ totalCount: TotalCount, entities: Items })  
      );
    })  
    .catch((error) => {
...
export const fetchIOTransactionType = (id) => (dispatch) => {
  if (!id) {
    return dispatch(actions.iOTransactionTypeFetched({ iOTransactionTypeForEdit: undefined }));
  }
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(actions.iOTransactionTypeFetched({ iOTransactionTypeForEdit: iOTransactionType }));
    })  
    .catch((error) => {
...
export const createIOTransactionType = (iOTransactionTypeForCreation) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
