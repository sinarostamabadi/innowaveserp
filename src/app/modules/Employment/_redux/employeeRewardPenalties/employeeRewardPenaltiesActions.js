...
import {
  employeeRewardPenaltiesSlice,
  callTypes,
} from "./employeeRewardPenaltiesSlice";
...
export const fetchEmployeeRewardPenalties = (queryParams) => (dispatch) => {
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.employeeRewardPenaltiesFetched({
...
export const fetchEmployeeRewardPenalty = (id) => (dispatch) => {
  if (!id) {
    return dispatch(
      actions.employeeRewardPenaltyFetched({
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.employeeRewardPenaltyFetched({
          employeeRewardPenaltyForEdit: employeeRewardPenalty,
        })
      );
    })
    .catch((error) => {
...
export const createEmployeeRewardPenalty =
  (employeeRewardPenaltyForCreation) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
...
export const updateEmployeeRewardPenalty =
  (employeeRewardPenalty) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
      .updateEmployeeRewardPenalty(employeeRewardPenalty)
      .then((response) => {
        dispatch(
          actions.employeeRewardPenaltyUpdated({ employeeRewardPenalty })
        );
      })
      .catch((error) => {
...
export const updateEmployeeRewardPenaltiesStatus =
  (ids, status) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
