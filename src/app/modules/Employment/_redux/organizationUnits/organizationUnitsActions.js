...
export const fetchOrganizationUnits = (queryParams) => (dispatch) => {
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(
        actions.organizationUnitsFetched({ totalCount: TotalCount, entities: Items })  
      );
    })  
    .catch((error) => {
...
export const fetchOrganizationUnit = (id) => (dispatch) => {
  if (!id) {
    return dispatch(actions.organizationUnitFetched({ organizationUnitForEdit: undefined }));
  }
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(actions.organizationUnitFetched({ organizationUnitForEdit: organizationUnit }));
    })  
    .catch((error) => {
...
export const createOrganizationUnit = (organizationUnitForCreation) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
