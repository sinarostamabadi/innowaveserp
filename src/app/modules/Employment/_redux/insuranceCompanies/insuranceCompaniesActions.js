...
export const fetchInsuranceCompanies = (queryParams) => (dispatch) => {
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(
        actions.insuranceCompaniesFetched({ totalCount: TotalCount, entities: Items })  
      );
    })  
    .catch((error) => {
...
export const fetchInsuranceCompany = (id) => (dispatch) => {
  if (!id) {
    return dispatch(actions.insuranceCompanyFetched({ insuranceCompanyForEdit: undefined }));
  }
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(actions.insuranceCompanyFetched({ insuranceCompanyForEdit: insuranceCompany }));
    })  
    .catch((error) => {
...
export const createInsuranceCompany = (insuranceCompanyForCreation) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
