...
import {
  physicalConditionTypesSlice,
  callTypes,
} from "./physicalConditionTypesSlice";
...
export const fetchPhysicalConditionTypes = (queryParams) => (dispatch) => {
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.physicalConditionTypesFetched({
...
export const fetchPhysicalConditionType = (id) => (dispatch) => {
  if (!id) {
    return dispatch(
      actions.physicalConditionTypeFetched({
        physicalConditionTypeForEdit: undefined,
      })
    );
  }
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.physicalConditionTypeFetched({
          physicalConditionTypeForEdit: physicalConditionType,
        })
      );
    })
    .catch((error) => {
...
export const createPhysicalConditionType =
  (physicalConditionTypeForCreation) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
...
export const updatePhysicalConditionType =
  (physicalConditionType) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
      .updatePhysicalConditionType(physicalConditionType)
      .then((response) => {
        dispatch(
          actions.physicalConditionTypeUpdated({ physicalConditionType })
        );
      })
      .catch((error) => {
...
export const updatePhysicalConditionTypesStatus =
  (ids, status) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
