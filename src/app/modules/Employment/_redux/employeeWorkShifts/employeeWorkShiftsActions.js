...
export const fetchEmployeeWorkShifts = (queryParams) => (dispatch) => {
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(
        actions.employeeWorkShiftsFetched({ totalCount: TotalCount, entities: Items })  
      );
    })  
    .catch((error) => {
...
export const fetchEmployeeWorkShift = (id) => (dispatch) => {
  if (!id) {
    return dispatch(actions.employeeWorkShiftFetched({ employeeWorkShiftForEdit: undefined }));
  }
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(actions.employeeWorkShiftFetched({ employeeWorkShiftForEdit: employeeWorkShift }));
    })  
    .catch((error) => {
...
export const createEmployeeWorkShift = (employeeWorkShiftForCreation) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
