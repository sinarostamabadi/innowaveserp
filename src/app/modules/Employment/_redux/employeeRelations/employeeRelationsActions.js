...
export const fetchEmployeeRelations = (queryParams) => (dispatch) => {
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(
        actions.employeeRelationsFetched({ totalCount: TotalCount, entities: Items })  
      );
    })  
    .catch((error) => {
...
export const fetchEmployeeRelation = (id) => (dispatch) => {
  if (!id) {
    return dispatch(actions.employeeRelationFetched({ employeeRelationForEdit: undefined }));
  }
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(actions.employeeRelationFetched({ employeeRelationForEdit: employeeRelation }));
    })  
    .catch((error) => {
...
export const createEmployeeRelation = (employeeRelationForCreation) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
