...
import {
  employeeLeaveChangesSlice,
  callTypes,
} from "./employeeLeaveChangesSlice";
...
export const fetchEmployeeLeaveChanges = (queryParams) => (dispatch) => {
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.employeeLeaveChangesFetched({
...
export const fetchEmployeeLeaveChange = (id) => (dispatch) => {
  if (!id) {
    return dispatch(
      actions.employeeLeaveChangeFetched({
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.employeeLeaveChangeFetched({
          employeeLeaveChangeForEdit: employeeLeaveChange,
        })
      );
    })
    .catch((error) => {
...
export const createEmployeeLeaveChange =
  (employeeLeaveChangeForCreation) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
...
export const updateEmployeeLeaveChange =
  (employeeLeaveChange) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
