...
export const organizationChartEmployeesSlice = createSlice({
  ...
  reducers: {
    ...
    organizationChartEmployeeFetched: (state, action) => {
      state.actionsLoading = false;
      state.organizationChartEmployeeForEdit =
        action.payload.organizationChartEmployeeForEdit;
      state.error = null;
    },
    ...
    organizationChartEmployeeUpdated: (state, action) => {
      ...
      state.entities = state.entities.map((entity) => {
        if (
          entity.OrganizationChartEmployeeId ===
          action.payload.organizationChartEmployee.OrganizationChartEmployeeId
        ) {
    ...
    organizationChartEmployeeDeleted: (state, action) => {
      ...
      state.entities = state.entities.filter(
        (el) =>
          el.OrganizationChartEmployeeId !==
          action.payload.OrganizationChartEmployeeId
      );
    },
    ...
    organizationChartEmployeesStatusUpdated: (state, action) => {
      ...
      state.entities = state.entities.map((entity) => {
        if (
          ids.findIndex((id) => id === entity.OrganizationChartEmployeeId) > -1
        ) {
