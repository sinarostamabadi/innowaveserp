...
export const fetchTechnicalType = (id) => (dispatch) => {
  if (!id) {
    return dispatch(
      actions.technicalTypeFetched({ technicalTypeForEdit: undefined })
    );
  }
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.technicalTypeFetched({ technicalTypeForEdit: technicalType })
      );
    })
    .catch((error) => {
