...
import {
  organizationChartLevelsSlice,
  callTypes,
} from "./organizationChartLevelsSlice";
...
export const fetchOrganizationChartLevels = (queryParams) => (dispatch) => {
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.organizationChartLevelsFetched({
...
export const fetchOrganizationChartLevel = (id) => (dispatch) => {
  if (!id) {
    return dispatch(
      actions.organizationChartLevelFetched({
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.organizationChartLevelFetched({
          organizationChartLevelForEdit: organizationChartLevel,
        })
      );
    })
    .catch((error) => {
...
export const createOrganizationChartLevel =
  (organizationChartLevelForCreation) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
...
export const updateOrganizationChartLevel =
  (organizationChartLevel) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
      .updateOrganizationChartLevel(organizationChartLevel)
      .then((response) => {
        dispatch(
          actions.organizationChartLevelUpdated({ organizationChartLevel })
        );
      })
      .catch((error) => {
...
export const updateOrganizationChartLevelsStatus =
  (ids, status) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
