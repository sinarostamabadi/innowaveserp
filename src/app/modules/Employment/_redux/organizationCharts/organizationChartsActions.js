...
export const fetchOrganizationChart = (id) => (dispatch) => {
  if (!id) {
    return dispatch(
      actions.organizationChartFetched({ organizationChartForEdit: undefined })
    );
  }
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.organizationChartFetched({
          organizationChartForEdit: organizationChart,
        })
      );
    })
    .catch((error) => {
...
export const createOrganizationChart =
  (organizationChartForCreation) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
