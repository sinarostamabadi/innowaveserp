...
import {
  employeeWorkExperiencesSlice,
  callTypes,
} from "./employeeWorkExperiencesSlice";
...
export const fetchEmployeeWorkExperiences = (queryParams) => (dispatch) => {
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.employeeWorkExperiencesFetched({
...
export const fetchEmployeeWorkExperience = (id) => (dispatch) => {
  if (!id) {
    return dispatch(
      actions.employeeWorkExperienceFetched({
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.employeeWorkExperienceFetched({
          employeeWorkExperienceForEdit: employeeWorkExperience,
        })
      );
    })
    .catch((error) => {
...
export const createEmployeeWorkExperience =
  (employeeWorkExperienceForCreation) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
...
export const updateEmployeeWorkExperience =
  (employeeWorkExperience) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
      .updateEmployeeWorkExperience(employeeWorkExperience)
      .then((response) => {
        dispatch(
          actions.employeeWorkExperienceUpdated({ employeeWorkExperience })
        );
      })
      .catch((error) => {
...
export const updateEmployeeWorkExperiencesStatus =
  (ids, status) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
