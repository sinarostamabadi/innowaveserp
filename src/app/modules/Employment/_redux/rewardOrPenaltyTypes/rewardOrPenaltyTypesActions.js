...
import {
  rewardOrPenaltyTypesSlice,
  callTypes,
} from "./rewardOrPenaltyTypesSlice";
...
export const fetchRewardOrPenaltyTypes = (queryParams) => (dispatch) => {
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.rewardOrPenaltyTypesFetched({
...
export const fetchRewardOrPenaltyType = (id) => (dispatch) => {
  if (!id) {
    return dispatch(
      actions.rewardOrPenaltyTypeFetched({
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.rewardOrPenaltyTypeFetched({
          rewardOrPenaltyTypeForEdit: rewardOrPenaltyType,
        })
      );
    })
    .catch((error) => {
...
export const createRewardOrPenaltyType =
  (rewardOrPenaltyTypeForCreation) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
...
export const updateRewardOrPenaltyType =
  (rewardOrPenaltyType) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
