...
import {
  paymentStatusesSlice,
  callTypes,
} from "./RestaurantInvoiceStatusesSlice";
...
export const fetchPaymentStatuses = (queryParams) => (dispatch) => {
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.paymentStatusesFetched({
...
export const fetchPaymentStatus = (id) => (dispatch) => {
  if (!id) {
    return dispatch(
      actions.paymentStatusFetched({ paymentStatusForEdit: undefined })
    );
  }
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.paymentStatusFetched({ paymentStatusForEdit: paymentStatus })
      );
    })
    .catch((error) => {
