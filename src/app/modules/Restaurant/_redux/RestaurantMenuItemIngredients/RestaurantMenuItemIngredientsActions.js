...
import {
  restaurantMenuItemIngredientsSlice,
  callTypes,
} from "./restaurantMenuItemIngredientsSlice";
const { actions } = restaurantMenuItemIngredientsSlice;
export const fetchRestaurantMenuItemIngredients =
  (queryParams) => (dispatch) => {
    ...
    return requestFromServer
      ...
      .then((response) => {
        ...
        dispatch(
          actions.restaurantMenuItemIngredientsFetched({
...
export const fetchRestaurantMenuItemIngredient = (id) => (dispatch) => {
  if (!id) {
    return dispatch(
      actions.restaurantMenuItemIngredientFetched({
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.restaurantMenuItemIngredientFetched({
          restaurantMenuItemIngredientForEdit: restaurantMenuItemIngredient,
        })
      );
    })
    .catch((error) => {
...
export const createRestaurantMenuItemIngredient =
  (restaurantMenuItemIngredientForCreation) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
      .createRestaurantMenuItemIngredient(
        restaurantMenuItemIngredientForCreation
      )
      .then((response) => {
        const restaurantMenuItemIngredient = response.data;
        dispatch(
          actions.restaurantMenuItemIngredientCreated(
...
export const updateRestaurantMenuItemIngredient =
  (restaurantMenuItemIngredient) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
      .updateRestaurantMenuItemIngredient(restaurantMenuItemIngredient)
      .then((response) => {
        dispatch(
          actions.restaurantMenuItemIngredientUpdated({
            restaurantMenuItemIngredient,
          })
        );
      })
...
export const updateRestaurantMenuItemIngredientsStatus =
  (ids, status) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
      ...
      .then(() => {
        dispatch(
          actions.restaurantMenuItemIngredientsStatusUpdated({ ids, status })
        );
      })
      .catch((error) => {
        error.clientMessage =
          "Can't update restaurantMenuItemIngredients status";
        dispatch(actions.catchError({ error, callType: callTypes.action }));
      });
  };
