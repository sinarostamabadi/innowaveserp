...
import {
  restaurantInvoiceCostsSlice,
  callTypes,
} from "./restaurantInvoiceCostsSlice";
...
export const fetchRestaurantInvoiceCosts = (queryParams) => (dispatch) => {
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.restaurantInvoiceCostsFetched({
...
export const fetchRestaurantInvoiceCost = (id) => (dispatch) => {
  if (!id) {
    return dispatch(
      actions.restaurantInvoiceCostFetched({
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.restaurantInvoiceCostFetched({
          restaurantInvoiceCostForEdit: restaurantInvoiceCost,
        })
      );
    })
    .catch((error) => {
...
export const createRestaurantInvoiceCost =
  (restaurantInvoiceCostForCreation) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
...
export const updateRestaurantInvoiceCost =
  (restaurantInvoiceCost) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
      .updateRestaurantInvoiceCost(restaurantInvoiceCost)
      .then((response) => {
        dispatch(
          actions.restaurantInvoiceCostUpdated({ restaurantInvoiceCost })
        );
      })
      .catch((error) => {
...
export const updateRestaurantInvoiceCostsStatus =
  (ids, status) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
