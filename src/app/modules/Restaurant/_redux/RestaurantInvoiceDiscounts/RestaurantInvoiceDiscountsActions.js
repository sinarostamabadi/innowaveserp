...
import {
  restaurantInvoiceDiscountsSlice,
  callTypes,
} from "./restaurantInvoiceDiscountsSlice";
...
export const fetchRestaurantInvoiceDiscounts = (queryParams) => (dispatch) => {
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.restaurantInvoiceDiscountsFetched({
...
export const fetchRestaurantInvoiceDiscount = (id) => (dispatch) => {
  if (!id) {
    return dispatch(
      actions.restaurantInvoiceDiscountFetched({
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.restaurantInvoiceDiscountFetched({
          restaurantInvoiceDiscountForEdit: restaurantInvoiceDiscount,
        })
      );
    })
    .catch((error) => {
...
export const createRestaurantInvoiceDiscount =
  (restaurantInvoiceDiscountForCreation) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
      ...
      .then((response) => {
        const restaurantInvoiceDiscount = response.data;
        dispatch(
          actions.restaurantInvoiceDiscountCreated(restaurantInvoiceDiscount)
        );
      })
      .catch((error) => {
...
export const updateRestaurantInvoiceDiscount =
  (restaurantInvoiceDiscount) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
      .updateRestaurantInvoiceDiscount(restaurantInvoiceDiscount)
      .then((response) => {
        dispatch(
          actions.restaurantInvoiceDiscountUpdated({
...
export const updateRestaurantInvoiceDiscountsStatus =
  (ids, status) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
      ...
      .then(() => {
        dispatch(
          actions.restaurantInvoiceDiscountsStatusUpdated({ ids, status })
        );
      })
      .catch((error) => {
