...
export const restaurantDiscountTypesSlice = createSlice({
  ...
  reducers: {
    ...
    restaurantDiscountTypeFetched: (state, action) => {
      state.actionsLoading = false;
      state.restaurantDiscountTypeForEdit = action.payload.restaurantDiscountTypeForEdit;
      state.error = null;
    },
    ...
    restaurantDiscountTypeUpdated: (state, action) => {
      ...
      state.entities = state.entities.map((entity) => {
        if (entity.RestaurantDiscountTypeId === action.payload.restaurantDiscountType.RestaurantDiscountTypeId) {
    ...
    restaurantDiscountTypeDeleted: (state, action) => {
      ...
      state.entities = state.entities.filter(
        (el) => el.RestaurantDiscountTypeId !== action.payload.RestaurantDiscountTypeId  
      );
    },
    ...
    restaurantDiscountTypesStatusUpdated: (state, action) => {
      ...
      state.entities = state.entities.map((entity) => {
        if (ids.findIndex((id) => id === entity.RestaurantDiscountTypeId) > -1) {
