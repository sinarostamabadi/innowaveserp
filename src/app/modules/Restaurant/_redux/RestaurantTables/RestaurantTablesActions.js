...
export const fetchRestaurantTables = (queryParams) => (dispatch) => {
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(
        actions.restaurantTablesFetched({ totalCount: TotalCount, entities: Items })  
      );
    })  
    .catch((error) => {
...
export const fetchRestaurantTable = (id) => (dispatch) => {
  if (!id) {
    return dispatch(actions.restaurantTableFetched({ restaurantTableForEdit: undefined }));
  }
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(actions.restaurantTableFetched({ restaurantTableForEdit: restaurantTable }));
    })  
    .catch((error) => {
...
export const createRestaurantTable = (restaurantTableForCreation) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
