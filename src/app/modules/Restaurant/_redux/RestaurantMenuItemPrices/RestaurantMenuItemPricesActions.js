...
import {
  restaurantMenuItemPricesSlice,
  callTypes,
} from "./restaurantMenuItemPricesSlice";
...
export const fetchRestaurantMenuItemPrices = (queryParams) => (dispatch) => {
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.restaurantMenuItemPricesFetched({
...
export const fetchRestaurantMenuItemPrice = (id) => (dispatch) => {
  if (!id) {
    return dispatch(
      actions.restaurantMenuItemPriceFetched({
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.restaurantMenuItemPriceFetched({
          restaurantMenuItemPriceForEdit: restaurantMenuItemPrice,
        })
      );
    })
    .catch((error) => {
...
export const createRestaurantMenuItemPrice =
  (restaurantMenuItemPriceForCreation) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
      ...
      .then((response) => {
        const restaurantMenuItemPrice = response.data;
        dispatch(
          actions.restaurantMenuItemPriceCreated(restaurantMenuItemPrice)
        );
      })
      .catch((error) => {
...
export const updateRestaurantMenuItemPrice =
  (restaurantMenuItemPrice) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
      .updateRestaurantMenuItemPrice(restaurantMenuItemPrice)
      .then((response) => {
        dispatch(
          actions.restaurantMenuItemPriceUpdated({ restaurantMenuItemPrice })
        );
      })
      .catch((error) => {
...
export const updateRestaurantMenuItemPricesStatus =
  (ids, status) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
      ...
      .then(() => {
        dispatch(
          actions.restaurantMenuItemPricesStatusUpdated({ ids, status })
        );
      })
      .catch((error) => {
