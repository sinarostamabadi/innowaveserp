...
import { placeOfPreparationsSlice, callTypes } from "./placeOfPreparationsSlice";
...
export const fetchPlaceOfPreparations = (queryParams) => (dispatch) => {
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(
        actions.placeOfPreparationsFetched({ totalCount: TotalCount, entities: Items })  
      );
    })  
    .catch((error) => {
...
export const fetchPlaceOfPreparation = (id) => (dispatch) => {
  if (!id) {
    return dispatch(actions.placeOfPreparationFetched({ placeOfPreparationForEdit: undefined }));
  }
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(actions.placeOfPreparationFetched({ placeOfPreparationForEdit: placeOfPreparation }));
    })  
    .catch((error) => {
...
export const createPlaceOfPreparation = (placeOfPreparationForCreation) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
