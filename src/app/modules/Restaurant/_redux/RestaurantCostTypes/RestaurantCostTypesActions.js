...
import { restaurantCostTypesSlice, callTypes } from "./restaurantCostTypesSlice";
...
export const fetchRestaurantCostTypes = (queryParams) => (dispatch) => {
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(
        actions.restaurantCostTypesFetched({ totalCount: TotalCount, entities: Items })  
      );
    })  
    .catch((error) => {
...
export const fetchRestaurantCostType = (id) => (dispatch) => {
  if (!id) {
    return dispatch(actions.restaurantCostTypeFetched({ restaurantCostTypeForEdit: undefined }));
  }
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(actions.restaurantCostTypeFetched({ restaurantCostTypeForEdit: restaurantCostType }));
    })  
    .catch((error) => {
...
export const createRestaurantCostType = (restaurantCostTypeForCreation) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
