...
import {
  restaurantMenuItemRatesSlice,
  callTypes,
} from "./restaurantMenuItemRatesSlice";
...
export const fetchRestaurantMenuItemRates = (queryParams) => (dispatch) => {
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.restaurantMenuItemRatesFetched({
...
export const fetchRestaurantMenuItemRate = (id) => (dispatch) => {
  if (!id) {
    return dispatch(
      actions.restaurantMenuItemRateFetched({
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.restaurantMenuItemRateFetched({
          restaurantMenuItemRateForEdit: restaurantMenuItemRate,
        })
      );
    })
    .catch((error) => {
...
export const createRestaurantMenuItemRate =
  (restaurantMenuItemRateForCreation) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
...
export const updateRestaurantMenuItemRate =
  (restaurantMenuItemRate) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
      .updateRestaurantMenuItemRate(restaurantMenuItemRate)
      .then((response) => {
        dispatch(
          actions.restaurantMenuItemRateUpdated({ restaurantMenuItemRate })
        );
      })
      .catch((error) => {
...
export const updateRestaurantMenuItemRatesStatus =
  (ids, status) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
