...
import {
  restaurantInvoiceDtlsSlice,
  callTypes,
} from "./restaurantInvoiceDtlsSlice";
...
export const fetchRestaurantInvoiceDtls = (queryParams) => (dispatch) => {
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.restaurantInvoiceDtlsFetched({
...
export const fetchRestaurantInvoiceDtl = (id) => (dispatch) => {
  if (!id) {
    return dispatch(
      actions.restaurantInvoiceDtlFetched({
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.restaurantInvoiceDtlFetched({
          restaurantInvoiceDtlForEdit: restaurantInvoiceDtl,
        })
      );
    })
    .catch((error) => {
...
export const createRestaurantInvoiceDtl =
  (restaurantInvoiceDtlForCreation) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
...
export const updateRestaurantInvoiceDtl =
  (restaurantInvoiceDtl) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
...
export const updateRestaurantInvoiceDtlsStatus =
  (ids, status) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
