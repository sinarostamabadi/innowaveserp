...
import { restaurantMenuGroupsSlice, callTypes } from "./restaurantMenuGroupsSlice";
...
export const fetchRestaurantMenuGroups = (queryParams) => (dispatch) => {
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(
        actions.restaurantMenuGroupsFetched({ totalCount: TotalCount, entities: Items })  
      );
    })  
    .catch((error) => {
...
export const fetchRestaurantMenuGroup = (id) => (dispatch) => {
  if (!id) {
    return dispatch(actions.restaurantMenuGroupFetched({ restaurantMenuGroupForEdit: undefined }));
  }
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(actions.restaurantMenuGroupFetched({ restaurantMenuGroupForEdit: restaurantMenuGroup }));
    })  
    .catch((error) => {
...
export const createRestaurantMenuGroup = (restaurantMenuGroupForCreation) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
...
export const updateRestaurantMenuGroup = (restaurantMenuGroup) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
