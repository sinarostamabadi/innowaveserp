...
export function RestaurantDiscountTypeDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // RestaurantDiscountTypes UI Context
  ...
  // RestaurantDiscountTypes Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.restaurantDiscountTypes.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteRestaurantDiscountType = () => {
    // server request for deleting restaurantDiscountType by id
    dispatch(actions.deleteRestaurantDiscountType(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("RestaurantDiscountType.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
      ...
      <Modal.Footer>
        <div>
          ...
          <button
            type="button"
            onClick={deleteRestaurantDiscountType}
