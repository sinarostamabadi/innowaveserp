...
export function PaymentStatusDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // PaymentStatuses UI Context
  ...
  // PaymentStatuses Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.paymentStatuses.actionsLoading }),
    shallowEqual
  );
  ...
  const deletePaymentStatus = () => {
    // server request for deleting paymentStatus by id
