...
export function RestaurantMenuItemPriceEdit({
  history,
  match: {
    params: { id },
  },
  ...
  const initModel = {
    RestaurantMenuItemPriceId: undefined,
    TitleFa: "",
    TitleEn: "",
  };
  ...
  useEffect(() => {
    dispatch(actions.fetchRestaurantMenuItemPrice(id));
  }, [id, dispatch]);
  ...
  }, [restaurantMenuItemPriceForEdit, id]);

  const saveRestaurantMenuItemPrice = (values) => {
    if (!id) {
      dispatch(actions.createRestaurantMenuItemPrice(values))
        .then((arg) => {
  ...
  const btnRef = useRef();
  const saveRestaurantMenuItemPriceClick = () => {
    if (btnRef && btnRef.current) {
      btnRef.current.click();
    }
  };

  const backToRestaurantMenuItemPricesList = () => {
    ...
    <Card>
      ...
      <CardHeader title={title}>
        <CardHeaderToolbar>
          <button
            type="button"
            onClick={backToRestaurantMenuItemPricesList}
          ...
          <button
            ...
            onClick={saveRestaurantMenuItemPriceClick}
          >
