...
import { RestaurantInvoiceDiscountEditForm } from "./RestaurantInvoiceDiscountEditForm";
...
export function RestaurantInvoiceDiscountEdit({
  history,
  match: {
    params: { id },
  },
  ...
  const initModel = {
    RestaurantInvoiceDiscountId: undefined,
    TitleFa: "",
    TitleEn: "",
  };
  ...
  }, [restaurantInvoiceDiscountForEdit, id]);

  const saveRestaurantInvoiceDiscount = (values) => {
    if (!id) {
      dispatch(actions.createRestaurantInvoiceDiscount(values))
        .then((arg) => {
          backToRestaurantInvoiceDiscountsList();
        })
        .catch((err) => {});
    } else {
      dispatch(actions.updateRestaurantInvoiceDiscount(id, values))
        .then(() => backToRestaurantInvoiceDiscountsList())
        .catch((err) => {});
    }
  };

  const btnRef = useRef();
  const saveRestaurantInvoiceDiscountClick = () => {
    if (btnRef && btnRef.current) {
      btnRef.current.click();
    }
  };

  const backToRestaurantInvoiceDiscountsList = () => {
    ...
    <Card>
      ...
      <CardHeader title={title}>
        <CardHeaderToolbar>
          <button
            type="button"
            onClick={backToRestaurantInvoiceDiscountsList}
          ...
          <button
            ...
            onClick={saveRestaurantInvoiceDiscountClick}
          >
