...
import { RestaurantCostTypeEditForm } from "./RestaurantCostTypeEditForm";
...
export function RestaurantCostTypeEdit({
  history,
  match: {
    params: { id },
  },
  ...
  const initModel = {
    RestaurantCostTypeId: undefined,
    TitleFa: "",
    TitleEn: "",
  };
  ...
  useEffect(() => {
    dispatch(actions.fetchRestaurantCostType(id));
  }, [id, dispatch]);
  ...
  }, [restaurantCostTypeForEdit, id]);

  const saveRestaurantCostType = (values) => {
    if (!id) {
      dispatch(actions.createRestaurantCostType(values))
        .then((arg) => {
  ...
  const btnRef = useRef();
  const saveRestaurantCostTypeClick = () => {
    if (btnRef && btnRef.current) {
      btnRef.current.click();
    }
  };

  const backToRestaurantCostTypesList = () => {
    ...
    <Card>
      ...
      <CardHeader title={title}>
        <CardHeaderToolbar>
          <button
            type="button"
            onClick={backToRestaurantCostTypesList}
          ...
          <button
            ...
            onClick={saveRestaurantCostTypeClick}
          >
      ...
      <CardBody>
        ...
        <div className="mt-5">
          {tab === "basic" && (
            <RestaurantCostTypeEditForm
              actionsLoading={actionsLoading}
              restaurantCostType={restaurantCostTypeForEdit || initModel}
              btnRef={btnRef}
              saveRestaurantCostType={saveRestaurantCostType}
            />
          )}
        </div>
      </CardBody>
