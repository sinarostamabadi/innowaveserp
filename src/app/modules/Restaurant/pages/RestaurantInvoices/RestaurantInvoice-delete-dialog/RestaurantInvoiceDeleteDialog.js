...
export function RestaurantInvoiceDeleteDialog({ id, show, onHide }) {
  ...
  const restaurantInvoicesUIProps = useMemo(() => {
    return {
      setIds: restaurantInvoicesUIContext.setIds,
  ...
  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.restaurantInvoices.actionsLoading }),
    shallowEqual
  );
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("RestaurantInvoice.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
      ...
      <Modal.Footer>
        <div>
          ...
          <button
            type="button"
            onClick={deleteRestaurantInvoice}
