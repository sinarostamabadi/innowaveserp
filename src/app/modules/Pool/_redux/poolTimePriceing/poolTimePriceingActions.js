...
export const fetchPoolTimePriceing = (queryParams) => (dispatch) => {
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(
        actions.poolTimePriceingFetched({ totalCount: TotalCount, entities: Items })  
      );
    })  
    .catch((error) => {
...
export const fetchPoolTimePriceing = (id) => (dispatch) => {
  if (!id) {
    return dispatch(actions.poolTimePriceingFetched({ poolTimePriceingForEdit: undefined }));
  }
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(actions.poolTimePriceingFetched({ poolTimePriceingForEdit: poolTimePriceing }));
    })  
    .catch((error) => {
      error.clientMessage = "Can't find poolTimePriceing";
...
export const createPoolTimePriceing = (poolTimePriceingForCreation) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
