...
import {
  billiardReservePricesSlice,
  callTypes,
} from "./billiardReservePricesSlice";
...
export const fetchBilliardReservePrices = (queryParams) => (dispatch) => {
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.billiardReservePricesFetched({
...
export const fetchBilliardReservePrice = (id) => (dispatch) => {
  if (!id) {
    return dispatch(
      actions.billiardReservePriceFetched({
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(
        actions.billiardReservePriceFetched({
          billiardReservePriceForEdit: billiardReservePrice,
        })
      );
    })
    .catch((error) => {
...
export const createBilliardReservePrice =
  (billiardReservePriceForCreation) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
...
export const updateBilliardReservePrice =
  (billiardReservePrice) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
...
export const updateBilliardReservePricesStatus =
  (ids, status) => (dispatch) => {
    dispatch(actions.startCall({ callType: callTypes.action }));
    return requestFromServer
