...
export const fetchBilliardCenters = (queryParams) => (dispatch) => {
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(
        actions.billiardCentersFetched({ totalCount: TotalCount, entities: Items })  
      );
    })  
    .catch((error) => {
...
export const fetchBilliardCenter = (id) => (dispatch) => {
  if (!id) {
    return dispatch(actions.billiardCenterFetched({ billiardCenterForEdit: undefined }));
  }
  ...
  return requestFromServer  
    ...
    .then((response) => {
      ...
      dispatch(actions.billiardCenterFetched({ billiardCenterForEdit: billiardCenter }));
    })  
    .catch((error) => {
...
export const createBilliardCenter = (billiardCenterForCreation) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer  
