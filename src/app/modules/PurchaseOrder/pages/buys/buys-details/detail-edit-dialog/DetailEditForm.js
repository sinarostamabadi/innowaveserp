...
export function DetailEditForm({ saveDetail, detail, actionsLoading, onHide }) {
  ...
  const DetailEditSchema = Yup.object().shape({
    ProductId: Yup.array().required(
      t("err.IsRequired", { 0: t("BuyDetail.Product") })
    ),
    ProductUnitId: Yup.string().required(
      t("err.IsRequired", { 0: t("BuyDetail.ProductUnit") })
    ),
    Amount: Yup.string().required(
      t("err.IsRequired", { 0: t("BuyDetail.Amount") })
    ),
    Price: Yup.string().required(
      t("err.IsRequired", { 0: t("BuyDetail.Price") })
    ),
  });
  ...
  useEffect(() => {
    if (!!detailsUIProps.selectedId && !!detail.BuyDetailId) {
  ...
  function cleanDetail(dirtyData) {
    return {
      BuyDetailId: dirtyData.BuyDetailId,
      BuyId: dirtyData.BuyId,
      BuyDetailRequestDetails: dirtyData.BuyDetailRequestDetails,
      Product:
            ...
            <Modal.Body className="">
              ...
              <Form className="form form-label-right">
                ...
                <div className="form-group row">
                  ...
                  <div className="col-lg-6">
                    <label>{t("BuyDetail.TotalPrice")}</label>
                ...
                <div className="form-group row">
                  <div className="col-lg-4">
                    <Field
                      name="DiscountPrice"
                      component={Input}
                      label={t("BuyDetail.DiscountPrice")}
                    />
                  </div>
                  <div className="col-lg-4">
                    <Field
                      ...
                      component={Input}
                      label={t("BuyDetail.DiscountPercent")}
                      onChange={(val) => {
                  ...
                  <div className="col-lg-4">
                    <label>{t("BuyDetail.TotalDiscount")}</label>
                    <label className="form-control">
                ...
                <div className="form-group row">
                  <div className="col-lg-6">
                    <Field
                      name="CostPrice"
                      component={Input}
                      label={t("BuyDetail.CostPrice")}
                    />
                  </div>
                  <div className="col-lg-6">
                    <label>{t("BuyDetail.PayablePrice")}</label>
                    <label className="form-control">
                      {numberWithCommas(
                        Math.ceil(
