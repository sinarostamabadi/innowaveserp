...
export function BuyCostDeleteDialog({ id, show, onHide }) {
  ...
  // BuyCosts Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.buyCosts.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteBuyCost = () => {
    // server request for deleting buyCost by id
    dispatch(actions.deleteBuyCost(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("BuyCost.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
