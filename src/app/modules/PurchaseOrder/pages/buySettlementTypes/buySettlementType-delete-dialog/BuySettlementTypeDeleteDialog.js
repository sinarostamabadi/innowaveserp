...
export function BuySettlementTypeDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // BuySettlementTypes UI Context
  ...
  // BuySettlementTypes Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.buySettlementTypes.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteBuySettlementType = () => {
    // server request for deleting buySettlementType by id
    dispatch(actions.deleteBuySettlementType(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("BuySettlementType.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
      ...
      <Modal.Footer>
        <div>
          ...
          <button
            type="button"
            onClick={deleteBuySettlementType}
