...
export function CostTypeDeleteDialog({ id, show, onHide }) {
  const { t } = useTranslation();

  // CostTypes UI Context
  ...
  // CostTypes Redux state
  const dispatch = useDispatch();

  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.costTypes.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteCostType = () => {
    // server request for deleting costType by id
    dispatch(actions.deleteCostType(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("CostType.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
