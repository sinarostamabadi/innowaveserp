...
export function BuyReturnDeleteDialog({ id, show, onHide }) {
  ...
  const { isLoading } = useSelector(
    (state) => ({ isLoading: state.buyReturns.actionsLoading }),
    shallowEqual
  );
  ...
  const deleteBuyReturn = () => {
    // server request for deleting buyReturn by id
    dispatch(actions.deleteBuyReturn(id))
      .then(() => {
  ...
  return (
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">{t("Common.Delete") + " " + t("BuyReturn.Entity")}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
