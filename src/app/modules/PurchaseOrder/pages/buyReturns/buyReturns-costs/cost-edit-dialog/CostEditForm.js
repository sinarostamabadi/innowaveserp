...
export function CostEditForm({ saveCost, cost, actionsLoading, onHide }) {
  ...
  const CostEditSchema = Yup.object().shape({
    CostTypeId: Yup.string().required(
      t("err.IsRequired", { 0: t("BuyReturnCost.CostType") })
    ),
    Price: Yup.string().required(
      t("err.IsRequired", { 0: t("BuyReturnCost.Price") })
    ),
  });
  ...
  const costsUIProps = useMemo(() => {
    return {
      buyReturnSum: costsUIContext.buyReturnSum,
  ...
  function cleanCost(dirtyData) {
    return {
      BuyReturnCostId: dirtyData.BuyReturnCostId,
              ...
              <Form className="form form-label-right">
                <div className="form-group row">
                  <div className="col">
                    <Select
                      name="CostTypeId"
                      label={t("BuyReturnCost.CostType")}
                    >
                ...
                <div className="form-group row">
                  ...
                  <div className="col">
                    <Field
                      name="CostPercent"
                      component={Input}
                      label={t("BuyReturnCost.CostPercent")}
                      onChange={(val) => {
                ...
                <div className="form-group row">
                  <div className="col">
                    <Field
                      name="Describtion"
                      component={Input}
                      label={t("BuyReturnCost.Describtion")}
                    />
                  </div>
                </div>
              </Form>
