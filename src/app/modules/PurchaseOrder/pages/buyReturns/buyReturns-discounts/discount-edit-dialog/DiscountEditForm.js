...
export function DiscountEditForm({
  ...
  const DiscountEditSchema = Yup.object().shape({
    DiscountTypeId: Yup.string().required(
      t("err.IsRequired", { 0: t("BuyReturnDiscount.DiscountType") })
    ),
    PricePercent: Yup.string().required(
      t("err.IsRequired", { 0: t("BuyReturnDiscount.PricePercent") })
    ),
  });
  ...
  const discountsUIProps = useMemo(() => {
    return {
      buyReturnSum: discountsUIContext.buyReturnSum,
  ...
  function cleanDiscount(dirtyData) {
    return {
      BuyReturnDiscountId: dirtyData.BuyReturnDiscountId,
              ...
              <Form className="form form-label-right">
                <div className="form-group row">
                  <div className="col">
                    <Select
                      name="DiscountTypeId"
                      label={t("BuyReturnDiscount.DiscountType")}
                    >
                ...
                <div className="form-group row">
                  <div className="col">
                    <Field
                      ...
                      component={Input}
                      type="text"
                      label={t("BuyReturnDiscount.DiscountPercent")}
                      onChange={(val) => {
                ...
                <div className="form-group row">
                  <div className="col">
                    <Field
                      name="PricePercent"
                      component={Input}
                      label={t("BuyReturnDiscount.PricePercent")}
                    />
                  </div>
                </div>
              </Form>
